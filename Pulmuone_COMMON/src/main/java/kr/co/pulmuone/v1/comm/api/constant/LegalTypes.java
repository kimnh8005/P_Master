...
public enum LegalTypes implements CodeCommEnum { // ( ERP API ) 법인코드 Enum
    ...
    private final String code; // ERP API 에서 전송하는 법인코드

    private final String codeName; /// 법인명

    // ERP API 에서 사용하는 해당 코드의 키 값 => Json 데이터 내에서 해당 키 값 사용
    public static final String CODE_KEY = "legCd";
    ...
    public static LegalTypes deserializeToEnumByCode( //
            @JsonProperty(LegalTypes.CODE_KEY) String code // ERP API 에서 전송한 법인코드
    ) {

        if (code == null) {
            return LegalTypes.NONE;
        }

        return EnumUtil.getEnum(LegalTypes.class, code);
