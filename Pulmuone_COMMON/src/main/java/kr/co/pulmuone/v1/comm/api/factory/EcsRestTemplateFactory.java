...
@Configuration(value = "ecsRestTemplateFactory")
@DependsOn(value = "httpRequestFactory") // CustomRestTemplate 클래스는 HttpRequestFactory 클래스에 의존
public class EcsRestTemplateFactory implements InitializingBean {

    @SuppressWarnings("unused") // 별도 log 이름 지정
    private Logger log = LoggerFactory.getLogger("EcsRestTemplateFactory");

    @Autowired
    private EcsHttpRequestFactory ecsHttpRequestFactory;
    ...
    public void afterPropertiesSet() throws Exception {
        this.ecsRestTemplate = new RestTemplate(ecsHttpRequestFactory.getHttpRequestFactory());

        ecsRestTemplate.setInterceptors(interceptors);
        ecsRestTemplate.setErrorHandler(errorHandler);
    }

    /*
     * RestTemplate 반환 메서드 : 각 통신마다 restTemplate 를 생성하지 않고 변수로 지정된 객체를 재활용
     */
    public RestTemplate getEcsRestTemplate() {
        return this.ecsRestTemplate;
    }
