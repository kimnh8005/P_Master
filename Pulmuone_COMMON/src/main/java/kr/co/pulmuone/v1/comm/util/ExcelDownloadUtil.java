...
public class ExcelDownloadUtil {
     ...
     * 엑셀 파일 생성
     *
     * @param excelDownloadDto ExcelDownloadDto
     * @param dataList         List<LinkedHashMap<String, Object>>
     * @return List<String>
     */
    public List<String> buildExcelDocument(ExcelDownloadDto excelDownloadDto, List<LinkedHashMap<String, Object>> dataList) throws Exception {
        List<String> returnDataArray = new ArrayList<>();
        SXSSFWorkbook workbook = new SXSSFWorkbook(1000); // turn off auto-flushing and accumulate all rows in memory
        ...
        for (ExcelWorkSheetDto workSheetDto : excelDownloadDto.getExcelWorkSheetList()) {
            ...
            // 해당 워크시트에 데이터 세팅
            setWorkSheetData(workSheetDto, workbook, worksheet, dataList);

            // 엑셀 다운로드 정보 세팅
            returnDataArray.add(setExcelFile(excelDownloadDto, workbook));
     ...
     *
     */
    private void setWorkSheetData(ExcelWorkSheetDto workSheetDto, Workbook workbook, Sheet worksheet, List<LinkedHashMap<String, Object>> dataList) {
...
//        }
        ...
        List<Map<String, Object>> mergeList1 = new ArrayList<>();
        ...
        for (int dataRowIdx = 0; dataRowIdx < dataRowCount; dataRowIdx++) {
            ...
            for (int columnIndex = 0; columnIndex < propertyCount; columnIndex++) {
                ...
                nodeValue = dataList.get(dataRowIdx).get(propertyList[columnIndex]);
...
//                    }
                    dataCellValue.append(nodeValue);
                }
                ...
                if (!befVal2.equals(curVal2)) {
                    ...
                    if (rowIdx > 0) {
                        ...
                        if ((int) mergeMap2.get("firstRow") != (int) mergeMap2.get("lastRow")) {
                            // [병합] 시작 로우와 종료 로우가 다를 경우
                            log.debug("# [2][firstRow][owIdx] 대상 :: " + "[" + mergeMap2.get("firstRow") + "][" + (int) mergeMap2.get("lastRow") + "]");
        ...
        // 테이블 하단 추가정보 Add
        // --------------------------------------------------------------------
        if (workSheetDto.getAddInfoList() != null && workSheetDto.getAddInfoList().size() > 0) {
     ...
     */
    private String setExcelFile(ExcelDownloadDto excelDownloadDto, SXSSFWorkbook workbook) {
        String fileFullPath = excelDownloadDto.getExcelFileName();
     ...
     * 공통 저장경로 + path getter
     */
    public String getCacheLocalFilePath(String path) {
