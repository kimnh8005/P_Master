...
public class GoodsSearchService extends SearchService
{
	...
	protected SearchResultDto search(GoodsSearchRequestDto searchRequestDto)
	{
		SearchResultDto goodsSearchResultDto = super.search(Indices.GOODS.getAlias(), makeQuery(searchRequestDto), GoodsSearchResultDto.class);

		if (!searchRequestDto.isFirstSearch()) return goodsSearchResultDto;

		return appendInitialFilterGroupBy(searchRequestDto, goodsSearchResultDto);
	 ...
	 * @param goodsSearchResultDto
	 * @return
	 * @throws Exception
	 */
	private SearchResultDto appendInitialFilterGroupBy(GoodsSearchRequestDto searchRequestDto, SearchResultDto goodsSearchResultDto) {
		SearchResultDto filterGroupByResultDto = super.search(Indices.GOODS.getAlias(), makeInitialFilterGroupByQuery(searchRequestDto, true), GoodsSearchResultDto.class);
		...
		// 위 검색 결과에 filter > mall 값을 mall_id 제외된 정보로 수정, 지금세일 페이지에서는 제외
		if(searchRequestDto.hasMallId() && !searchRequestDto.isDiscountGoods()) {
			SearchResultDto filterMallGroupByResultDto = super.search(Indices.GOODS.getAlias(), makeInitialFilterGroupByQuery(searchRequestDto, false), GoodsSearchResultDto.class);
		...
		goodsSearchResultDto.setFilter(filterGroupByResultDto.getFilter());
	 ...
	 * @return
	 */
	private SearchSourceBuilder makeInitialFilterGroupByQuery(GoodsSearchRequestDto searchRequestDto, boolean isMallCategory)
	{
		BoolQueryBuilder query = QueryBuilders.boolQuery();

		if (searchRequestDto.hasKeyword()) {
			QueryBuilder multiKeywordQuery = QueryBuilders.multiMatchQuery(searchRequestDto.getKeyword(), Indices.GOODS.getMultiMatchFields()).operator(Operator.AND);
	 ...
	 * @throws Exception
	 */
	private SearchSourceBuilder makeQuery(GoodsSearchRequestDto searchRequestDto) {
		BoolQueryBuilder query = QueryBuilders.boolQuery();
		query = appendCategoryQuery(query, searchRequestDto);
	 ...
	 * @return
	 */
	private BoolQueryBuilder appendFilterQuery(BoolQueryBuilder query, GoodsSearchRequestDto searchRequestDto)
	{
		...
		{
			RangeQueryBuilder priceRangeQuery =  searchRequestDto.isEmployee() ?
								new RangeQueryBuilder("employee_discount_price").gte(searchRequestDto.getMinimumPrice()).lte(searchRequestDto.getMaximumPrice())
								: new RangeQueryBuilder("sale_price").gte(searchRequestDto.getMinimumPrice()).lte(searchRequestDto.getMaximumPrice());
	 ...
	 * @return
	 */
	private BoolQueryBuilder appendCategoryQuery(BoolQueryBuilder query, GoodsSearchRequestDto searchRequestDto)
	{
	 ...
	 * @throws Exception
	 */
	private BoolQueryBuilder appendCategoryBoostQuery(BoolQueryBuilder query, GoodsSearchRequestDto searchRequestDto) {
	...
	private SearchResultDto convertMultiValueFilter(SearchResultDto goodsSearchResultDto)
	{
		goodsSearchResultDto.getFilter().keySet().forEach(k -> {
			List<AggregationDocumentDto> list = (List<AggregationDocumentDto>) goodsSearchResultDto.getFilter().get(k);
			...
			goodsSearchResultDto.getFilter().put(k, list);
		});
		return goodsSearchResultDto;
	}
