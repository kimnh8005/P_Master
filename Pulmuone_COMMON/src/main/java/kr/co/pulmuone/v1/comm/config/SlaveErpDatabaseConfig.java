...
@MapperScan(
        ...
        basePackages = {
                "kr.co.pulmuone.v1.comm.mappers.slaveErp"
        }
)
public class SlaveErpDatabaseConfig {
    @Value("${spring.datasource.hikari.slave-erp.mybatis.config-location}")
    private String mybatisConfigLocation;

    @Bean(name = "slaveErpDataSource")
    @ConfigurationProperties(prefix = "spring.datasource.hikari.slave-erp")
    ...
    public SqlSessionFactory slaveErpSqlSessionFactory(@Qualifier("slaveErpDataSource") DataSource dataSource) throws Exception {
        ...
        Resource[] arrResource = resolver.getResources("classpath*:/persistence/mappers/slaveErp/**/*.xml");
    ...
    public SqlSessionTemplate slaveErpSqlSessionTemplate(
            @Qualifier("slaveErpSqlSessionFactory") SqlSessionFactory slaveErpSqlSessionFactory) {
        return new SqlSessionTemplate(slaveErpSqlSessionFactory);
    }

    @Bean(name = "slaveErpTransactionManager")
    public DataSourceTransactionManager slaveErpTransactionManager(
            @Qualifier("slaveErpDataSource") DataSource slaveErpDataSource) {
