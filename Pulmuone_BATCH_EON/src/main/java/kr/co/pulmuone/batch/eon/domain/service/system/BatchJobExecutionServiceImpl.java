...
import kr.co.pulmuone.batch.eon.domain.model.system.BatchStatus;
import kr.co.pulmuone.batch.eon.domain.service.system.exception.BatchJobNotFinishedException;
import kr.co.pulmuone.batch.eon.domain.service.system.exception.BatchJobNotStartedException;
import kr.co.pulmuone.batch.eon.domain.service.system.exception.NotExistsBatchNoException;
import kr.co.pulmuone.batch.eon.domain.service.system.exception.NotExistsJobExecutionException;
import kr.co.pulmuone.batch.eon.infra.mapper.system.BatchJobExecutionMapper;
...
public class BatchJobExecutionServiceImpl implements BatchJobExecutionService {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public BatchJobExecution failJobExecutionLastJob(long batchNo) {

        BatchJobExecution batchJobExecution = mapper.findFirstByBatchNoOrderByJobExecutionIdDesc(batchNo);

        if(batchJobExecution == null || batchJobExecution.getJobExecutionId() == null) return null;

        return setBatchJobExecution(getBatchJobExecution(batchJobExecution.getJobExecutionId(), batchNo).fail("FAILED UPDATE!!"));
    }
...
//   }
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public BatchJobExecution setBatchJobExecutionLastJob(BatchJobExecution batchJobExecution) {
        mapper.setBatchJobExecution(batchJobExecution);
        return Optional.of(mapper.findByJobExecutionIdAndBatchNo(batchJobExecution.getJobExecutionId(), batchJobExecution.getBatchNo()))
                .orElseThrow(NotExistsJobExecutionException::new);
    }
