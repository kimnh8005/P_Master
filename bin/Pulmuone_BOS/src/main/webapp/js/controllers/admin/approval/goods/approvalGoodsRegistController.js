/**-----------------------------------------------------------------------------
 * description : 상품등록 승인목록 처리
 * @
 * @ 아래 상품등록 공통 처리
 * @  - 상품등록 승인요청 내역 : approvalGoodsRegistRequestController.js, approvalGoodsRegistRequest.html
 * @  - 상품등록 승인관리 : approvalGoodsRegistAcceptController.js  , approvalGoodsRegistAccept.html
 * @
 * @ 수정일			수정자		수정내용
 * @ ---------------------------------------------------------------------------
 * @ 2021.02.17		박승현		최초생성
...
$(document).ready(function() {
	...
	function fnInitialize(){
	    ...
	    // ------------------------------------------------------------------------
	    // 상품등록 승인 요청 목록, 승인 대상 목록 Set
	    //   - approvalGoodsRegistRequestController.js 에서 설정 됨
		//   - approvalGoodsRegistAcceptController.js 에서 설정 됨
	    // ------------------------------------------------------------------------
	    ...
	    // ------------------------------------------------------------------------
	    if(!fnIsEmpty(pageId)) {
	    	if (pageId == 'approvalGoodsRegistRequest') {
	    		$('#pageTitleSpan').text('상품등록 승인요청 내역');
	    	}else if (pageId == 'approvalGoodsRegistAccept') {
	    		$('#pageTitleSpan').text('상품등록 승인관리');
	    	}
	    }
	};
...
//	            viewModel.searchInfo.set("searchCondition", "GOODS_CODE"); // 상품코드 설정
		        ...
		        pageParam.ilGoodsId = null; //처음만 검색되도록 pageParam.ilGoodsId를 초기화 한다.
			}
...
//		$('#searchForm').formClear(true);
    ...
    function fnDefaultSet(){
        ...
        if(!(PG_SESSION == null || PG_SESSION == '' || PG_SESSION == undefined || PG_SESSION == 'undefined')) {
			...
			if(pageId == 'approvalGoodsRegistAccept')
    ...
    function fnViewModelInit(){
        viewModel = new kendo.data.ObservableObject({
            searchInfo : { // 조회조건
            	...
            	apprKindType : "APPR_KIND_TP.GOODS_REGIST", 		// 승인코드
...
//		}
	...
	function fnInitGrid(){
		...
		aGridDs = fnGetPagingDataSource({
			url      : "/admin/approval/goods/getApprovalGoodsRegistList",
			pageSize : PAGE_SIZE
		});
			...
			columns : [
						{ field : 'chk', headerTemplate : "<input type='checkbox' id='checkBoxAll' />", width:'50px', attributes : {style : "text-align:center;"}
							, template : function(dataItem){
								...
								else if (pageId == 'approvalGoodsRegistRequest') {
									if (dataItem.apprStat == 'APPR_STAT.REQUEST') {
										if (
												PG_SESSION != undefined && PG_SESSION != null && PG_SESSION.loginId != undefined && PG_SESSION.loginId != null
						...
						, {field : "goodsId", title : "상품코드", width : "70px", attributes : { style : "text-align:center;color:blue;" }}
						, {field : "goodsTypeName", title : "상품유형", width : "70px", attributes : { style : "text-align:center" } }
						, {field : "goodsName", title : "상품명", width : "290px", attributes : { style : "text-align:center;color:blue;" }
							, template : function(dataItem){
							...
							,command: [ { text: '요청철회', imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon",
										...
										, { text: '승인완료'		, imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon",
											...
											,visible: function(dataItem) {
												...
												if (pageId != 'approvalGoodsRegistAccept') // 승인관리 페이지에서만
													return false;
	...
	function fnGridClick(e){
        ...
        let goodsTypeCode = dataItem.goodsTypeCode;
    ...
    function fnApprovalProcess(params){
    	...
    	fnKendoMessage({
			...
			ok      : function(){
		    	fnAjax({
		    		url     : "/admin/approval/goods/putApprovalProcessGoodsRegist",
		    		params  : params,
