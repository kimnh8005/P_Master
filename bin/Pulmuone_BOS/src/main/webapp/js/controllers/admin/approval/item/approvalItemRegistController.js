/**-----------------------------------------------------------------------------
 * description : 품목등록 승인목록 처리
 * @
 * @ 아래 품목등록 공통 처리
 * @  - 품목등록 승인요청 내역 : approvalItemRegistRequestController.js, approvalItemRegistRequest.html
 * @  - 품목등록 승인관리 : approvalItemRegistAcceptController.js  , approvalItemRegistAccept.html
 * @
 * @ 수정일			수정자		수정내용
 * @ ---------------------------------------------------------------------------
 * @ 2021.02.17		박승현		최초생성
...
$(document).ready(function() {
	...
	function fnInitialize(){
	    ...
	    // ------------------------------------------------------------------------
	    // 품목등록 승인 요청 목록, 승인 대상 목록 Set
	    //   - approvalItemRegistRequestController.js 에서 설정 됨
		//   - approvalItemRegistAcceptController.js 에서 설정 됨
	    // ------------------------------------------------------------------------
	    ...
	    // ------------------------------------------------------------------------
	    if(!fnIsEmpty(pageId)) {
	    	if (pageId == 'approvalItemRegistRequest') {
	    		$('#pageTitleSpan').text('품목등록 승인요청 내역');
	    	}else if (pageId == 'approvalItemRegistAccept') {
	    		$('#pageTitleSpan').text('품목등록 승인관리');
	    	}
	    }
...
//		$('#searchForm').formClear(true);
	...
	function fnDefaultSet(){
		...
		if(!(PG_SESSION == null || PG_SESSION == '' || PG_SESSION == undefined || PG_SESSION == 'undefined')) {
			...
			if(pageId == 'approvalItemRegistAccept')
	...
	function fnViewModelInit() {
		viewModel = new kendo.data.ObservableObject({
            searchInfo : { // 조회조건
            	...
            	itemName : "", 			// 품목명
            	apprKindType : "APPR_KIND_TP.ITEM_REGIST", 		// 승인코드
...
//            cscdField  : "warehouseGroupCode"
	...
	function fnInitGrid() {
		...
		aGridDs = fnGetPagingDataSource({
			url : '/admin/approval/item/getApprovalItemRegistList',
			pageSize : PAGE_SIZE,
				...
				, columns : [
						{ field : 'chk', headerTemplate : "<input type='checkbox' id='checkBoxAll' />", width:'50px', attributes : {style : "text-align:center;"}
							, template : function(dataItem){
								...
								else if (pageId == 'approvalItemRegistRequest') {
									if (dataItem.apprStat == 'APPR_STAT.REQUEST') {
										if (
											PG_SESSION != undefined && PG_SESSION != null && PG_SESSION.loginId != undefined && PG_SESSION.loginId != null
						...
						, {field : 'ilItemBarcode',  title : '품목코드/<br>품목바코드', width : '140px', attributes : { style : 'text-align:center;color:blue;' }
							, template: function(dataItem) {
						...
						, {field : 'itemName', title : '마스터 품목명', width : '250px', attributes : { style : 'text-align:center;color:blue;' } }
							...
							,command: [ { text: '요청철회', imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon",
										...
										, { text: '승인완료'		, imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon",
											click: function(e) {  e.preventDefault();
																	var tr = $(e.target).closest("tr");
																	var data = this.dataItem(tr);
																	fnConfirmApproval(data.ilItemApprId);}
											, visible: function(dataItem) {
												var hasAuth = false;

												if (pageId != 'approvalItemRegistAccept') // 승인관리 페이지에서만
													return false;
										...
										, { text: '승인반려' , imageClass: "k-i-add", className: "f-grid-search k-margin5", iconClass: "k-icon",
											click: function(e) {  e.preventDefault();
																	var tr = $(e.target).closest("tr");
																	var data = this.dataItem(tr);
																	fnDenieApprovalPopup(data.ilItemApprId);
												            	}
											, visible: function(dataItem) {
												var hasAuth = false;
	...
	function fnGetSelectType(ilItemApprId) {
	...
	//승인 반려사유 팝업
	function fnDenieApprovalPopup(ilItemApprId){
		fnKendoInputPoup({height:"400px" ,width:"500px", title:{ nullMsg :'승인 반려 사유' } });
		fnGetSelectType(ilItemApprId);
	}

	//승인처리
	function fnConfirmApproval(ilItemApprId) {
		let params = {};
		let selectType = fnGetSelectType(ilItemApprId);
    ...
    function fnApprovalProcess(params){
    	...
    	fnKendoMessage({
			...
			ok      : function(){
		    	fnAjax({
		    		url     : "/admin/approval/item/putApprovalProcessItemRegist",
		    		params  : params,
