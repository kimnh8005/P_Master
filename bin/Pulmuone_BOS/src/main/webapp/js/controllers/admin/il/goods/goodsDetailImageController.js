...
var PAGE_SIZE = 20;
var viewModel, goodsDetailImageGridDs, goodsDetailImageGridOpt, goodsDetailImageGrid;
...
//		viewModel.fnGridDataSort();
	...
	function fnInitGrid(){
		...
		goodsDetailImageGridDs = fnGetPagingDataSource({
			url	  : "/admin/goods/list/getGoodsDetailImageList",
			pageSize : PAGE_SIZE
		});

		goodsDetailImageGridOpt = {
			dataSource: goodsDetailImageGridDs,
			pageable  : { pageSizes: [20, 30, 50, 100], buttonCount : 10 },
			navigatable : true,
			height : 755,
			scrollable : true,
			columns : [ { field : "modifyDate", title : "업데이트 일자", width : "10%", attributes : {style : "text-align:center;"} }
						, { field : "goodsId", title : "상품코드", width : "10%", attributes : { style : "text-align:center; text-decoration: underline;color:blue;" } }
						, { field : "itemCode", title : "마스터 품목코드/<BR>품목바코드", width : "10%", attributes : {style : "text-align:center;"},
						  template : function(dataItem){
							  ...
							  if( dataItem.itemBarcode != "" && dataItem.itemBarcode != null ){
						...
						, { field : "goodsName", title : "상품명", width : "40%", attributes : { style : "text-align:center; text-decoration: underline;color:blue;" } }
						...
						, { field : "updateContents", title : "업데이트 항목", width : "15%", attributes : { style : "text-align:center" },
						    template : function(dataItem){
                              let returnValue = '';

                              if( dataItem.chgGoodsDetlYn == "Y" && dataItem.chgSpceYn == "Y"){
                                    returnValue += "상품상세정보" + "/<br>" + "상품고시정보";
                              } else if( dataItem.chgGoodsDetlYn == "Y" && dataItem.chgSpceYn == "N"){
                                    returnValue += "상품상세정보";
                              } else if( dataItem.chgGoodsDetlYn == "N" && dataItem.chgSpceYn == "Y"){
                                    returnValue += "상품고시정보";
                              }
                              return returnValue;
						    }
						...
						, { field : "management", title : "관리", width : "10%", attributes : { style : "text-align:center" }
							, template: function(dataItem) {
									return	'<div id="buttonArea" class="textCenter">'
											+ '<button type="button" class="k-button k-button-icontext" kind="btnImageView">미리보기</button>' + '&nbsp;'
											+ '<button type="button" class="k-button k-button-icontext" kind="btnImageDown">다운로드</button>'
											+ '</div>';
							}
						  }
			]
		...
		goodsDetailImageGrid = $('#goodsDetailImageGrid').initializeKendoGrid( goodsDetailImageGridOpt ).cKendoGrid();

		goodsDetailImageGrid.bind("dataBound", function() {
			$('#countTotalSpan').text( kendo.toString(goodsDetailImageGridDs._total, "n0") );
		});


		$("#goodsDetailImageGrid").on("click", "tbody>tr>td", function () {
			var index = $(this).index();
			var aMap = goodsDetailImageGrid.dataItem(goodsDetailImageGrid.select());
	...
	function fnInitOptionBox(){
		...
		fnTagMkRadio({
            id: 'searchUpdateType',
            tagId: 'searchUpdateType',
            data: [{'CODE': '', 'NAME': '전체'},
                {'CODE': 'GOODS_DETAIL', 'NAME': '상품상세정보'},
                {'CODE': 'GOODS_NOTICE', 'NAME': '상품고시정보'}
            ],
            chkVal: '',
            style: {}
        });
...
//		fnInputValidationForHangulAlphabetNumber("goodsName"); // 상품명
		...
		viewModel = new kendo.data.ObservableObject({
			...
			fnSearch : function(e){ // 조회
				...
				if(!itemNameLengthChk) {
					const _pageSize = goodsDetailImageGrid && goodsDetailImageGrid.dataSource ? goodsDetailImageGrid.dataSource.pageSize() : PAGE_SIZE;
					...
					goodsDetailImageGridDs.query(query);
					initSearch = false;
				}
			},
			...
			fnExcelDownload : function(e){ // 엑셀다운로드
				...
				fnExcelDownload('/admin/goods/list/getGoodsDetailImageListExportExcel', data);
			},
    ...
    // 상품 상세 이미지 미리보기
	$('#goodsDetailImageGrid').on('click', 'button[kind=btnImageView]', function(e) {
		e.preventDefault();
		let dataItem = goodsDetailImageGrid.dataItem($(e.currentTarget).closest('tr'));

		let ilGoodsId = dataItem.goodsId;
		let createDate = dataItem.createDate;

        let url = CUR_SERVER_URL + "/shop/externalMallPreview?goods="+ilGoodsId;

        $("#detailImagePreview").attr('src',url);
        fnKendoInputPoup({id:"kendoPopupImagePreview",height:"900px" ,width:"1000px",title:{nullMsg :'상품 상세 이미지 미리보기'} });

        $("#kendoPopupImagePreview").data('kendoWindow').bind("activate", function(e){
            $("#kendoPopupImagePreview").css('overflow','hidden');
        });

	});

    // 상품 상세 이미지 파일 다운로드
	$('#goodsDetailImageGrid').on('click', 'button[kind=btnImageDown]', function(e) {
    		e.preventDefault();
    		let dataItem = goodsDetailImageGrid.dataItem($(e.currentTarget).closest('tr'));

    		let ilGoodsId = dataItem.ilGoodsId;
    		let createDate = dataItem.createDate;
    		let imageNm = dataItem.pcImgNm;   // ex) //BOS/il/goods/imageName/2021/10/08/PC_150012_20211008102000.png

            var urlEnd = imageNm.lastIndexOf('/');
            var filePath = imageNm.substring(0,urlEnd);
            var fileName = imageNm.substring(urlEnd+1, imageNm.length);
