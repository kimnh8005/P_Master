...
$(document).ready(function() {
    ...
    function fnExcelDown(){
	   ...
	   fnExcelDownload('/admin/goods/stock/getStockErpExportExcel', data);
	...
	function fnInitGrid(){
		aGridDs = fnGetPagingDataSource({
			url      : '/admin/goods/stock/getStockErpList',
			pageSize : PAGE_SIZE
		});
		aGridOpt = {
			dataSource: aGridDs
			...
			,columns   : [
				...
				,{ field:'ilItemCd'			,title : '품목코드/<br/>품목바코드', width:'90px'	,attributes:{ style:'text-align:center' }
											...
											if(dataItem.ilItemCd != "" && dataItem.barcode != ""){
												...
												var html = "<a kind='itemDetlInfo' style='color:blue;cursor:pointer'>" +
													       kendo.htmlEncode(dataItem.ilItemCd)+"/-";
											}
				...
				,{ field:'stockTpNm'		,title : '타입'		    , width:'90px'	,attributes:{ style:'text-align:center' }}
	...
	function fnInitOptionBox(){
		...
		fnTagMkRadio({
			...
			chkVal: '',
			data  : [   { "CODE" : ""	                    , "NAME":'전체' },
						{ "CODE" : "ERP_STOCK_TP.CLOSED"    , "NAME":'전일마감' },
						{ "CODE" : "ERP_STOCK_TP.SCHEDULED"	, "NAME":'입고예정' },
						{ "CODE" : "ERP_STOCK_TP.CONFIRMED"	, "NAME":'입고확정' }
					],
			style : {}
	    });
