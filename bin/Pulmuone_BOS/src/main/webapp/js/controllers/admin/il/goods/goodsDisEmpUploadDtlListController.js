/**-----------------------------------------------------------------------------
 * description 		 : 임직원상품 할인 일괄 업로드 상세 내역
 * @
 * @ 수정일			수정자          수정내용
 * @ ------------------------------------------------------
...
//                  toggleElement(mode);
	...
	function fnItemGrid() {
		...
		itemGridDs = fnGetEditPagingDataSource({
			url : '/admin/goods/discount/getGoodsDiscountEmpUploadDtlList',
			pageSize : PAGE_SIZE,
		...
		itemGridOpt = {
			...
			,	{ field : 'compNm', title : '공급업체', width : '140px', attributes : { style : 'text-align:center' } }
	...
	function fnExcelExport() {
		var data = $('#searchForm').formSerialize(true);
		fnExcelDownload('/admin/goods/discount/createGoodsDiscountEmpUploadDtlListExcel', data);
	}
