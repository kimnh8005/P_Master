...
$(document).ready(function() {
	importScript("/js/service/od/claim/claimInfoExcelUploadListGridColumns.js", function() {
	...
	function fnInitialize(){
		...
		fnPageInfo({
			PG_ID  : 'claimInfoExcelUpload',
			callback : fnUI
		});
	}

	function fnUI(){
		getPolicyClaimBosList();
...
//              fnExcelUpload(file, fileTagId);
	...
	function fnExcelUpload(file, fileTagId) {
		...
		$.ajax({
			url         : '/admin/claim/addClaimExcelUpload'
			, data        : formData
			, type        : 'POST'
			, contentType : false
			, processData : false
	...
	function fnSampleDownload(){
		document.location.href = "/contents/excelsample/order/claimExcelUploadSample.xlsx"
	}
	...
	function fnInitGrid(){
		aGridDs = fnGetPagingDataSource({
			url      : "/admin/claim/getClaimExcelInfoList",
			pageSize : PAGE_SIZE
		});
		aGridOpt = {
			dataSource: aGridDs
			...
			,navigatable: true
			,columns   : ifDayUploadListGridUtil.claimUploadList()
		};
        ...
        $('#aGrid').on("click", "button[kind=btnExcelDown]", function(e) {
            ...
            let data = { "ifClaimExcelInfoId" : dataItem.ifClaimExcelInfoId, "failType" : $(this).data('failtype')};
            fnExcelDownload('/admin/claim/getClaimFailExcelDownload', data, btnObj);
        });
	...
	function getPolicyClaimBosList(){
		fnAjax({
			url     : '/admin/policy/claim/getPolicyClaimBosList',
			success :
				function( data ){
					let html = "";
					html += "<tr name='viewHideArea'>"
					html += "<th scope='row' style='text-align: center;'>NO</th>"
					html += "<th scope='row' style='text-align: center;'>BOS 클레임사유 Code</th>"
					html += "<th scope='row' style='text-align: center;'>클레임 사유(대)</th>"
					html += "<th scope='row' style='text-align: center;'>클레임 사유(중)</th>"
					html += "<th scope='row' style='text-align: center;'>귀책처</th>"
					html += "</tr>"
					for(let i=0; i<data.rows.length; i++){
						...
						if(vo.useYn == "Y"){
							...
							html += "<td style='text-align: center;'>" + vo.psClaimBosId + "</td>";
							html += "<td style='text-align: center;'>" + vo.lclaimCtgryName + "</td>";
							html += "<td style='text-align: center;'>" + vo.mclaimCtgryName + "</td>";
							var targetType = vo.targetType == 'B' ? '(구매자 귀책)' : '(판매자 귀책)' ;
							html += "<td style='text-align: center;'>" + vo.sclaimCtgryName + targetType + "</td>";
							html += "</tr>";
						}
					}
					$("#claimContent").html(html);
					$("tr[name=viewHideArea]").hide();
				},
			isAction : 'select'
		});
	...
	$scope.fnViewHideAreaChange = function(){	fnViewHideAreaChange();	};
