...
$(document).ready(function() {
	...
	function fnDuplicateGoods(){
		...
		let paramGoodsType = "GOODS_TYPE.INCORPOREITY";
			...
			fnAjax({
				...
				success	: function(data){
					...
					if(data.ilGoodsId){
						...
						fnInitGoodsIncorporeal();			// Kendo Component 초기화
						init();
	...
	function fnViewModelSetting(){
		...
		viewModel = new kendo.data.ObservableObject({															//Kendo UI MVVM Pattern 적용
			...
			ilGoodsDetail : {
				...
				goodsType							: 'GOODS_TYPE.INCORPOREITY',										// 상품 유형(goodsIncorporeal 페이지는 무조건 무형상품 페이지 이므로 고정값으로 저장)
				goodsTypeName						: '무형',													// 코드에 따라서 불러오도록 변경 해야 함
				ilGoodsId							: '',														// 상품코드
				goodsCreateDate						: '',														// 상품 등록일자
				confirmStatus						: '',														// 최근 수정일
				mdRecommendYn						: 'N',														// MD추천 노출여부 DropwonList
				goodsName							: '',														// 상품명
				packageUnitDisplayYn				: 'N',														// 상품명 > 포장용량 구성정보 > 노출 여부 > 기본설정값 // 무형상품은 기본값 N
				...
				autoDisplaySizeYn					: 'N',						// 단위별 용량정보 > 자동표기 여부 radio buttion // 무형상품의 경우 단위별 용량정보 기본값은 N
				sizeEtc								: '',						// 단위별 용량정보 > 자동표기안함 일때 textbox
				isAutoSizeValue						: false,						// 단위별 용량정보 > 자동표기안함 일때 textbox visible // 무형상품의 경우 기본값은 false
			...
			fnOrderIfDateCalc : function(e) {		//주문수집I/F 일자 계산
				...
				if (isPassedIFfOrderDatesCutofftime(reservationEndDateTime)) { // 예약주문 가능 종료일의 마감시간이 주문마감시간 이후면 다음날 부터
				...
				success	: function(data){
					if(data.ilGoodsDetail == null){
						...
						if(data.goodsDiscountErpEventList.length > 0){
							...
							viewModel.ilGoodsDetail.set("isGoodsDiscountErpEventListNoDataTbody", false);							//가격 정보 > 행사/할인 내역 > ERP행사 > Nodata 항목 // 무형상품은 항상 비노출
							viewModel.ilGoodsDetail.set("isGoodsDiscountErpEventListTbody", false);									//가격 정보 > 행사/할인 내역 > ERP행사 > Data 항목 // 무형상품은 항상 비노출
						}
						else {
						...
						if(data.goodsReservationOptionList){
							for(var i=0; i < data.goodsReservationOptionList.length; i++){
								...
								$("#ifCalc-btn-id"+goodsReservationOption.saleSequance).attr("class", "btn-ligntgray");
...
//						fnApprHtml(data.ilGoodsDetail);
	...
	// Kendo Component 초기화
	function fnInitGoodsIncorporeal() {
						...
						success : function(data, result) {
							fnKendoMessage({
								...
								ok : function() {
									...
									if(pageMode == "create") {
										var successUrl = "/layout.html#/goodsIncorporeal?ilGoodsId="+data['ilGoodsId'];
										location.href = successUrl;
									}
									else{
