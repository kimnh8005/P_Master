...
$(document).ready(function() {
	...
	function fnInitGrid(){
		aGridDs = fnGetPagingDataSource({
			url      : '/admin/st/log/getConnectLogList',
			pageSize : PAGE_SIZE
		});
		aGridOpt = {
			dataSource: aGridDs
			...
			,columns   : [
				...
				,{ field:'loginId'		, title : '접속ID'	   		, width:'60px',attributes:{ style:'text-align:center' }}
				,{ field:'userName'		, title : '접속자'	   			, width:'60px',attributes:{ style:'text-align:center' }}
				,{ field:'ip'			, title : '접속IP'	   		, width:'60px',attributes:{ style:'text-align:center' }}
				,{ field:'serviceName'	, title : '서비스'				, width:'60px',attributes:{ style:'text-align:center' }}
				,{ field:'successYn'	, title : '구분'				, width:'60px',attributes:{ style:'text-align:center' }
					, template: "#=(successYn=='Y') ? '성공' : '실패'#"}
				,{ field:'agent'		, title : 'User-Agent'		, width:'300px',attributes:{ style:'text-align:center' }}
				,{ field:'loginDate'	, title : '로그인 일자(등록일자)'	, width:'80px',attributes:{ style:'text-align:center' }}
				,{ field:'logoutDate'	, title : '로그아웃 일자'	    , width:'80px',attributes:{ style:'text-align:center' }}
			],
			change : fnGridClick
		};
	...
	function fnInitOptionBox(){
		fnKendoDropDownList({
			...
			params : {"stCommonCodeMasterCode" : "ACCESS_LOG_DIV", "useYn" :"Y"},
		});
		fnKendoDropDownList({
			id  : 'device',
			url : "/admin/comn/getCodeList",
		...
		fnKendoDropDownList({
			id  : 'service',
			url : "/admin/comn/getCodeList",
			params : {"stCommonCodeMasterCode" : "SERVICE_TYPE", "useYn" :"Y"},
			blank : "All"
		});
		fnKendoDatePicker({
			id    : 'startCreateDate',
