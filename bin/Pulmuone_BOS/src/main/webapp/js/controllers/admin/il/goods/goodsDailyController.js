...
$(document).ready(function() {
			...
			fnAjax({
				...
				success	: function(data){
					...
					if(data.ilGoodsId){
						...
						init();
						fnExhibitGiftGrid();		// 혜택/구매정보 > 증정 행사 Grid > 추후 kendo template로 대체 처리할 예정
	...
	function fnDefaultValue() {
		if(ilGoodsId != undefined && ilGoodsId != ""){	//상품 수정이라면
            ...
            $('#mealScheduleAdmin').attr('disabled', true);
		}
	...
	function fnViewModelSetting(){
		...
		viewModel = new kendo.data.ObservableObject({															//Kendo UI MVVM Pattern 적용
			...
			ilGoodsDetail : {
				...
				goodsType							: 'GOODS_TYPE.DAILY',										// 상품 유형(goodsMgm 페이지는 무조건 일반상품 페이지 이므로 고정값으로 저장)
				goodsTypeName						: '일일',													// 코드에 따라서 불러오도록 변경 해야 함
				ilGoodsId							: '',														// 상품코드
				...
				/* HGRM-1646 초기화 및 시간달력포맷 공통화 수정
				saleStartDate						: '2020-08-13 23:59',			//판매 기간 > 시작일
				saleEndDate							: '2099-08-13 23:59',			//판매 기간 > 종료일
				*/

				// saleStartDate						: defaultStartTime,												// 판매 기간 > 시작 기간
				// saleEndDate							: defaultEndTime,												// 판매 기간 >  종료 기간
				// saleStartYear						: kendo.toString(kendo.parseDate(defaultStartTime), "yyyy-MM-dd"),	// 판매 기간 >  시작일
				// saleStartHour						: "00",//kendo.toString(kendo.parseDate(defaultStartTime), "HH"),		// 판매 기간 >  시작 시간
				// saleStartMinute						: "00",//kendo.toString(kendo.parseDate(defaultStartTime), "mm"),		// 판매 기간 >  시작 분
				// saleEndYear							: kendo.toString(kendo.parseDate(defaultEndTime), "yyyy-MM-dd"),	// 판매 기간 >  종료일
				// saleEndHour							: kendo.toString(kendo.parseDate(defaultEndTime), "HH"),		// 판매 기간 >  종료 시간
				// saleEndMinute						: kendo.toString(kendo.parseDate(defaultEndTime), "mm"),		// 판매 기간 >  종료 분
				...
				/* 판매 정보 Start */
				babyMealVisible							: false,							// 판매유형 > 일일 판매 옵션 설정 > 베이비밀 관련 항목 Visible
				eatSlimVisible							: false,							// 판매유형 > 일일 판매 옵션 설정 > 잇슬림 관련 항목 Visible
				goodsDailyType							: 'GOODS_DAILY_TP.GREENJUICE',		// 판매유형 > 일반판매(기본설정)
				greenJuiceCleanseOpt					: false,							// 판매유형 > 녹즙_클렌즈 옵션사용
				goodsDailyAllergyYn						: 'N',								// 판매유형 > 일일 판매 옵션 설정 > 알러지 식단 포함여부
				goodsCycleType7Day						: ['GOODS_CYCLE_TP.7DAYS_PER_WEEK'],	//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단
				goodsCycleTermType7Day					: [									//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단 > 주차
					'GOODS_CYCLE_TERM_TP.WEEK1'
				,	'GOODS_CYCLE_TERM_TP.WEEK2'
				,	'GOODS_CYCLE_TERM_TP.WEEK4'
				],
				goodsCycleType6Day						: ['GOODS_CYCLE_TP.6DAYS_PER_WEEK'],	//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단
				goodsCycleTermType6Day					: [									//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단 > 주차
					'GOODS_CYCLE_TERM_TP.WEEK1'
				,	'GOODS_CYCLE_TERM_TP.WEEK2'
				,	'GOODS_CYCLE_TERM_TP.WEEK4'
				],
				goodsCycleType5Day						: ['GOODS_CYCLE_TP.5DAYS_PER_WEEK'],	//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단
				goodsCycleTermType5Day					: [									//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단 > 주차
					'GOODS_CYCLE_TERM_TP.WEEK1'
				,	'GOODS_CYCLE_TERM_TP.WEEK2'
				,	'GOODS_CYCLE_TERM_TP.WEEK4'
				],
				goodsCycleType3Day						: ['GOODS_CYCLE_TP.3DAYS_PER_WEEK'],	//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단
				goodsCycleTermType3Day					: [									//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
					'GOODS_CYCLE_TERM_TP.WEEK2'
				,	'GOODS_CYCLE_TERM_TP.WEEK4'
				],
				goodsCycleTypeEatslim3Day				: ['GOODS_CYCLE_TP.3DAYS_PER_WEEK'],	//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단(잇슬림)
				goodsCycleTermTypeEatslim3Day			: [									//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단(잇슬림) > 주차
					'GOODS_CYCLE_TERM_TP.WEEK2'
				,	'GOODS_CYCLE_TERM_TP.WEEK4'
				],

				goodsDailyBulkYn							: 'Y',							//판매 정보 > 일일 판매 옵션 설정 > 일괄 배달 설정 > 허용 여부
				goodsBulkType							: [									//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
					'GOODS_BULK_TP.7SETS'
				],
				goodsBulkTypeVisible					: true,								//판매 정보 > 일일 판매 옵션 설정 > 일괄 배달 설정 > 세팅 Visible
...
//				goodsReservationOptionList				: [],							// 판매유형(예약판매) > 예약 판매 옵션 설정 세부 항목 목록, goodsReservationOption-row-template 사용
				...
				saleShopYn								: 'N',							// 판매정보 > 판매유형 > 매장판매
				/* 판매 정보 End */

				/* 식단 정보 Start */

                mealScheduleList		: [],							            // 식단 정보
                mealScheduleListNoDataVisible : true,					            // 식단 정보 정보 > Nodata Tbody Visible
                mealScheduleListDataVisible : false,					            // 식단 정보 정보 > Data Tbody Visible
			...
			onGoodsCycleType7DayChange : function(e){				//판매 정보 > 판매 유형 > 일일 판매 옵션 설정 > 식단주기선택(7일 식단)에 따른 주 체크 전체 선택/해제
				e.preventDefault();
				if(e.target.checked){
					viewModel.ilGoodsDetail.set("goodsCycleTermType7Day"
							, ['GOODS_CYCLE_TERM_TP.WEEK1'
								, 'GOODS_CYCLE_TERM_TP.WEEK2'
								, 'GOODS_CYCLE_TERM_TP.WEEK4']);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단 > 주차
				}
				else{
					viewModel.ilGoodsDetail.set("goodsCycleTermType7Day", []);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단 > 주차
				}
			},

			onGoodsCycleTermType7DayChange : function(e){
				e.preventDefault();

				var dayLength = $("input[name=" + e.target.name + "]").length;					//주차 전체 갯수
				var dayCheckedLength = $("input[name=" + e.target.name + "]:checked").length;	//주차 선택된 갯수

				if(dayCheckedLength == 0) {														//선택된 주차가 하나도 없다면 주기 checkbox도 체크 해제 처리
					viewModel.ilGoodsDetail.set("goodsCycleType7Day", []);
				}
				else {																			//하나라도 선택된 주차가 있다면 주기 checkbox도 체크 처리
					viewModel.ilGoodsDetail.set("goodsCycleType7Day", ["GOODS_CYCLE_TP.7DAYS_PER_WEEK"]);
				}
			},

			onGoodsCycleType6DayChange : function(e){				//판매 정보 > 판매 유형 > 일일 판매 옵션 설정 > 식단주기선택(6일 식단)에 따른 주 체크 전체 선택/해제
				e.preventDefault();
				if(e.target.checked){
					viewModel.ilGoodsDetail.set("goodsCycleTermType6Day"
							, ['GOODS_CYCLE_TERM_TP.WEEK1'
								, 'GOODS_CYCLE_TERM_TP.WEEK2'
								, 'GOODS_CYCLE_TERM_TP.WEEK4']);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단 > 주차
				}
				else{
					viewModel.ilGoodsDetail.set("goodsCycleTermType6Day", []);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단 > 주차
				}
			},

			onGoodsCycleTermType6DayChange : function(e){
				e.preventDefault();
				...
				if(dayCheckedLength == 0) {														//선택된 주차가 하나도 없다면 주기 checkbox도 체크 해제 처리
					viewModel.ilGoodsDetail.set("goodsCycleType6Day", []);
				}
				else {																			//하나라도 선택된 주차가 있다면 주기 checkbox도 체크 처리
					viewModel.ilGoodsDetail.set("goodsCycleType6Day", ["GOODS_CYCLE_TP.6DAYS_PER_WEEK"]);
				}
			},

			onGoodsCycleType5DayChange : function(e){				//판매 정보 > 판매 유형 > 일일 판매 옵션 설정 > 식단주기선택(5일 식단)에 따른 주 체크 전체 선택/해제
				e.preventDefault();
				if(e.target.checked){
					viewModel.ilGoodsDetail.set("goodsCycleTermType5Day"
							, ['GOODS_CYCLE_TERM_TP.WEEK1'
								, 'GOODS_CYCLE_TERM_TP.WEEK2'
								, 'GOODS_CYCLE_TERM_TP.WEEK4']);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단 > 주차
				}
				else{
					viewModel.ilGoodsDetail.set("goodsCycleTermType5Day", []);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단 > 주차
				}
			},

			onGoodsCycleTermType5DayChange : function(e){
				e.preventDefault();
				...
				if(dayCheckedLength == 0) {														//선택된 주차가 하나도 없다면 주기 checkbox도 체크 해제 처리
					viewModel.ilGoodsDetail.set("goodsCycleType5Day", []);
				}
				else {																			//하나라도 선택된 주차가 있다면 주기 checkbox도 체크 처리
					viewModel.ilGoodsDetail.set("goodsCycleType5Day", ["GOODS_CYCLE_TP.5DAYS_PER_WEEK"]);
				}
			},

			onGoodsCycleType3DayChange : function(e){				//판매 정보 > 판매 유형 > 일일 판매 옵션 설정 > 식단주기선택(3일 식단)에 따른 주 체크 전체 선택/해제
				e.preventDefault();
				if(e.target.checked){
					viewModel.ilGoodsDetail.set("goodsCycleTermType3Day"
							, ['GOODS_CYCLE_TERM_TP.WEEK2'
								, 'GOODS_CYCLE_TERM_TP.WEEK4']);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
				}
				else{
					viewModel.ilGoodsDetail.set("goodsCycleTermType3Day", []);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
				}
			},

			onGoodsCycleTermType3DayChange : function(e){
				e.preventDefault();
				...
				if(dayCheckedLength == 0) {														//선택된 주차가 하나도 없다면 주기 checkbox도 체크 해제 처리
					viewModel.ilGoodsDetail.set("goodsCycleType3Day", []);
				}
				else {																			//하나라도 선택된 주차가 있다면 주기 checkbox도 체크 처리
					viewModel.ilGoodsDetail.set("goodsCycleType3Day", ["GOODS_CYCLE_TP.3DAYS_PER_WEEK"]);
				}
			},

			ongoodsDailyBulkYnChange : function(e){					//판매 정보 > 판매 유형 > 일일 판매 옵션 설정 > 일괄 배달 설정 > 허용 여부에 따른 Visible
				switch(viewModel.ilGoodsDetail.goodsDailyBulkYn) {
					case 'Y' :
						viewModel.ilGoodsDetail.set('goodsBulkTypeVisible', true);
						$("input:checkbox[value='GOODS_BULK_TP.7SETS']").prop("checked", true);
						$("#goodsDailyBulkRowspan").attr("rowspan", 2);
						break;
					case 'N' :
						viewModel.ilGoodsDetail.set('goodsBulkTypeVisible', false);
						$("#goodsDailyBulkRowspan").attr("rowspan", 1);
						break;
				}
			},

			onGoodsCycleTypeEatslim3DayChange : function(e){				//판매 정보 > 판매 유형(잇슬림) > 일일 판매 옵션 설정 > 식단주기선택(3일 식단)에 따른 주 체크 전체 선택/해제
				e.preventDefault();
				if(e.target.checked){
					viewModel.ilGoodsDetail.set("goodsCycleTermTypeEatslim3Day"
							, ['GOODS_CYCLE_TERM_TP.WEEK1'
								, 'GOODS_CYCLE_TERM_TP.WEEK2'
								, 'GOODS_CYCLE_TERM_TP.WEEK4']);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
				}
				else{
					viewModel.ilGoodsDetail.set("goodsCycleTermTypeEatslim3Day", []);		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
				}
			},

			onGoodsCycleTermTypeEatslim3DayChange : function(e){
				e.preventDefault();
				...
				if(dayCheckedLength == 0) {														//선택된 주차가 하나도 없다면 주기 checkbox도 체크 해제 처리
					viewModel.ilGoodsDetail.set("goodsCycleTypeEatslim3Day", []);
				}
				else {																			//하나라도 선택된 주차가 있다면 주기 checkbox도 체크 처리
					viewModel.ilGoodsDetail.set("goodsCycleTypeEatslim3Day", ["GOODS_CYCLE_TP.3DAYS_PER_WEEK"]);
				}
			},
			...
			fnAjax({
				...
				success	: function(data){
					if(data.ilGoodsDetail == null){
						...
						//판매정보 > 판매 유형 Visible
						switch(viewModel.ilGoodsDetail.goodsDailyType) {
							case 'GOODS_DAILY_TP.GREENJUICE' :
								viewModel.ilGoodsDetail.set('isGoodsDailyType', false);
								$("#goodsDailyTypeTh").attr("rowspan",1);
								break;
							case 'GOODS_DAILY_TP.BABYMEAL' :
								viewModel.ilGoodsDetail.set('isGoodsDailyType', false);
								$("#goodsDailyTypeTh").attr("rowspan",2);
								break;
							case 'GOODS_DAILY_TP.EATSSLIM' :
								viewModel.ilGoodsDetail.set('isGoodsDailyType', true);
								$("#goodsDailyTypeTh").attr("rowspan",2);
								break;
						}

						//console.log("goodsDailyCyclelist", data.goodsDailyCyclelist);
						//판매정보 > 일일 판매 옵션 설정 > 식단주기선택
						if(data.goodsDailyCyclelist && data.goodsDailyCyclelist.length > 0){

							var goodsCycleType7Day = [];
							var goodsCycleType6Day = [];
							var goodsCycleType5Day = [];
							var goodsCycleType3Day = [];
							var goodsCycleTypeEatslim3Day = [];

							var goodsCycleTermType7DayArry = [];
							var goodsCycleTermType6DayArry = [];
							var goodsCycleTermType5DayArry = [];
							var goodsCycleTermType3DayArry = [];
							var goodsCycleTermTypeEatslim3DayArry = [];

							for(var i in data.goodsDailyCyclelist){
								if(data.goodsDailyCyclelist[i].goodsCycleType == "GOODS_CYCLE_TP.7DAYS_PER_WEEK"){
									goodsCycleType7Day.push(data.goodsDailyCyclelist[i].goodsCycleType);
									goodsCycleTermType7DayArry.push(data.goodsDailyCyclelist[i].goodsCycleTermType);
								}

								if(data.goodsDailyCyclelist[i].goodsCycleType == "GOODS_CYCLE_TP.6DAYS_PER_WEEK"){
									goodsCycleType6Day.push(data.goodsDailyCyclelist[i].goodsCycleType);
									goodsCycleTermType6DayArry.push(data.goodsDailyCyclelist[i].goodsCycleTermType);
								}

								if(data.goodsDailyCyclelist[i].goodsCycleType == "GOODS_CYCLE_TP.5DAYS_PER_WEEK"){
									goodsCycleType5Day.push(data.goodsDailyCyclelist[i].goodsCycleType);
									goodsCycleTermType5DayArry.push(data.goodsDailyCyclelist[i].goodsCycleTermType);
								}

								if(viewModel.ilGoodsDetail.goodsDailyType == "GOODS_DAILY_TP.BABYMEAL" && data.goodsDailyCyclelist[i].goodsCycleType == "GOODS_CYCLE_TP.3DAYS_PER_WEEK"){
									goodsCycleType3Day.push(data.goodsDailyCyclelist[i].goodsCycleType);
									goodsCycleTermType3DayArry.push(data.goodsDailyCyclelist[i].goodsCycleTermType);
								}

								/* HGRM-8542 잇슬림 격일패턴 제거 - 값이 세팅되지 않도록 주석 처리
								if(viewModel.ilGoodsDetail.goodsDailyType == "GOODS_DAILY_TP.EATSSLIM" && data.goodsDailyCyclelist[i].goodsCycleType == "GOODS_CYCLE_TP.3DAYS_PER_WEEK"){
									goodsCycleTypeEatslim3Day.push(data.goodsDailyCyclelist[i].goodsCycleType);
									goodsCycleTermTypeEatslim3DayArry.push(data.goodsDailyCyclelist[i].goodsCycleTermType);
								}
								*/
							}
							viewModel.ilGoodsDetail.set('goodsCycleType7Day', goodsCycleType7Day);
							viewModel.ilGoodsDetail.set('goodsCycleType6Day', goodsCycleType6Day);
							viewModel.ilGoodsDetail.set('goodsCycleType5Day', goodsCycleType5Day);
							viewModel.ilGoodsDetail.set('goodsCycleType3Day', goodsCycleType3Day);
							viewModel.ilGoodsDetail.set('goodsCycleTypeEatslim3Day', goodsCycleTypeEatslim3Day);

							viewModel.ilGoodsDetail.set('goodsCycleTermType7Day', goodsCycleTermType7DayArry);
							viewModel.ilGoodsDetail.set('goodsCycleTermType6Day', goodsCycleTermType6DayArry);
							viewModel.ilGoodsDetail.set('goodsCycleTermType5Day', goodsCycleTermType5DayArry);
							viewModel.ilGoodsDetail.set('goodsCycleTermType3Day', goodsCycleTermType3DayArry);
							viewModel.ilGoodsDetail.set('goodsCycleTermTypeEatslim3Day', goodsCycleTermTypeEatslim3DayArry);

							//console.log("goodsCycleType7Day", viewModel.ilGoodsDetail.get("goodsCycleType7Day"));
						}
						else{
							viewModel.ilGoodsDetail.set('goodsCycleType7Day', []);
							viewModel.ilGoodsDetail.set('goodsCycleType6Day', []);
                        ...
                        // 식단 정보 표시 처리 (공급업체 : PDM / 표준브랜드 : 베이비밀, 잇슬림)
                        if((data.ilGoodsDetail.urCompanyId == '6' && data.ilGoodsDetail.mallinmallCategoryId == 'MALL_DIV.BABYMEAL') ||
                            (data.ilGoodsDetail.urCompanyId == '6' && data.ilGoodsDetail.mallinmallCategoryId == 'MALL_DIV.EATSLIM')){

                            $('#mealInfoView').show();

                            if(data.mealScheduleList && data.mealScheduleList.length > 0){
                                viewModel.ilGoodsDetail.set("mealScheduleList", data.mealScheduleList);
                                viewModel.ilGoodsDetail.set("mealScheduleListNoDataVisible", false);
                                viewModel.ilGoodsDetail.set("mealScheduleListDataVisible", true);

                                $('#mealScheduleAdmin').attr('disabled', false);
                            }else {
                                viewModel.ilGoodsDetail.set("mealScheduleList", []);
                                viewModel.ilGoodsDetail.set("mealScheduleListNoDataVisible", true);
                                viewModel.ilGoodsDetail.set("mealScheduleListDataVisible", false);
						...
						if(data.goodsDailyBulklist && data.goodsDailyBulklist.length > 0){
							var goodsDailyBulkArry = [];

							for(var i in data.goodsDailyBulklist) {
								goodsDailyBulkArry.push(data.goodsDailyBulklist[i].goodsBulkType);
							}

							viewModel.ilGoodsDetail.set("goodsBulkType", goodsDailyBulkArry);
						}
						else{
							viewModel.ilGoodsDetail.set("goodsBulkType", []);
						}
						...
						switch(viewModel.ilGoodsDetail.goodsDailyBulkYn) {
							...
							case 'N' :
								viewModel.ilGoodsDetail.set("goodsBulkTypeVisible", false);
								$("input:radio[name='goodsDailyBulkYn'][value='N']").prop("checked", true);
								$("#goodsDailyBulkRowspan").attr("rowspan", 1);
								break;
						}
...
//						fnApprHtml(data.ilGoodsDetail);
		...
		fnAjax({
			...
			success	:
				function( data ){
					...
					//판매 정보 > 판매유형, 일일 판매 옵션 설정(해당 Component는 품목에서 가져온 정보를 가지고 처리 해야 하므로 따로 처리)
					fnSaleType(data.row.supplierCode, data.row.mallinmallCategoryId);

					/* 상세 정보 항목 Start */

					//console.log("data.row.supplierName : ", data.row.supplierName);
					...
					//console.log("fnMasterItemAll > itemPriceList", viewModel.ilGoodsDetail.get("itemPriceList"));
						...
						fnAjax({
							...
							success	:
								function( data ){
									...
									itemCertificationHtml += "		<col style='width: 10%'>";
									itemCertificationHtml += "		<col style='width: 15%'>";
									itemCertificationHtml += "		<col style='width: 75%'>";
									itemCertificationHtml += "	</colgroup>";
									...
									if(data.rows.length>0){
										for(var i=0; i < data.rows.length; i++){
											itemCertificationHtml += "	<tr>";
											// PJH Start
											itemCertificationHtml += "		<th scope='row'><img src='" + publicStorageUrl + data.rows[i].imagePath + data.rows[i].imageName + "' style='width:130px;height:130px;'></th>";
											// PJH End
	...
	function fnItemPriceList(ilItemCode){
		fnAjax({
			...
			isAction : 'select',
			success : function(data, status, xhr) {
				//console.log("fnItemPriceList > data.itemPriceList", data.itemPriceList);
				if(data != null){
	...
	function fnItemWarehouseList(goodsShippingTemplateList){
		fnAjax({
			...
			params : {
				ilItemCode : ilItemCode
			,	urWarehouseId : urWarehouseId
			,	goodsType : viewModel.ilGoodsDetail.goodsType
			},
			isAction : 'select',
			success : function(data, status, xhr) {
...
//			max: lastDate,
		...
		$("input[name=saleStartYear]").each(function() {
...
//			max: lastDate,
		...
		$("input[name=saleEndYear]").each(function() {
			$(this).addClass("clientAllow");
		});
		...
		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단
		fnTagMkChkBox({
			id : 'goodsCycleType7Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TP.7DAYS_PER_WEEK",
				"NAME" : "주7일 식단(월-일)"
			} ],
			tagId : 'goodsCycleType7Day'
		});
		$("input[name=goodsCycleType7Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleType7Day, events: {change: onGoodsCycleType7DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단 > 주차
		fnTagMkChkBox({
			id : 'goodsCycleTermType7Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK1",
				"NAME" : "1주"
			}, {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK2",
				"NAME" : "2주"
			}, {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK4",
				"NAME" : "4주"
			} ],
			tagId : 'goodsCycleTermType7Day'
		});
		$("input[name=goodsCycleTermType7Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleTermType7Day, events: {change: onGoodsCycleTermType7DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단
		fnTagMkChkBox({
			id : 'goodsCycleType6Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TP.6DAYS_PER_WEEK",
				"NAME" : "주6일 식단(월-토)"
			} ],
			tagId : 'goodsCycleType6Day'
		});
		$("input[name=goodsCycleType6Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleType6Day, events: {change: onGoodsCycleType6DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단 > 주차
		fnTagMkChkBox({
			id : 'goodsCycleTermType6Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK1",
				"NAME" : "1주"
			}, {
			...
			tagId : 'goodsCycleTermType6Day'
		});
		$("input[name=goodsCycleTermType6Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleTermType6Day, events: {change: onGoodsCycleTermType6DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단
		fnTagMkChkBox({
			id : 'goodsCycleType5Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TP.5DAYS_PER_WEEK",
				"NAME" : "주5일 식단(월-토)"
			} ],
			tagId : 'goodsCycleType5Day'
		});
		$("input[name=goodsCycleType5Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleType5Day, events: {change: onGoodsCycleType5DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단 > 주차
		fnTagMkChkBox({
			id : 'goodsCycleTermType5Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK1",
				"NAME" : "1주"
			}, {
				...
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK8",
				"NAME" : "8주"
			} ],
			tagId : 'goodsCycleTermType5Day'
		});
		$("input[name=goodsCycleTermType5Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleTermType5Day, events: {change: onGoodsCycleTermType5DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단
		fnTagMkChkBox({
			id : 'goodsCycleType3Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TP.3DAYS_PER_WEEK",
				"NAME" : "주3일 식단(월/수/금)"
			} ],
			tagId : 'goodsCycleType3Day'
		});
		$("input[name=goodsCycleType3Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleType3Day, events: {change: onGoodsCycleType3DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
		fnTagMkChkBox({
			id : 'goodsCycleTermType3Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK2",
				"NAME" : "2주"
			}, {
			...
			tagId : 'goodsCycleTermType3Day'
		});
		$("input[name=goodsCycleTermType3Day]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleTermType3Day, events: {change: onGoodsCycleTermType3DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 일괄 배달 설정 > 허용 여부
		fnTagMkRadio({
			id : 'goodsDailyBulkYn',
			data : [ {
				"CODE" : "Y",
		...
		$("input[name=goodsDailyBulkYn]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsDailyBulkYn, events: {change: ongoodsDailyBulkYnChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 일괄 배달 설정 > 세트
		fnTagMkChkBox({
			id : 'goodsBulkType',
			data : [ {
				"CODE" : "GOODS_BULK_TP.7SETS",
				"NAME" : "7세트"
			} ],
			tagId : 'goodsBulkType'
		});
		$("input[name=goodsBulkType]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsBulkType");
		});
		$("input[name=goodsBulkType]").bind("click", false);


		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단(잇슬림)
		fnTagMkChkBox({
			id : 'goodsCycleTypeEatslim3Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TP.3DAYS_PER_WEEK",
				"NAME" : "주3일 식단(월/수/금)"
			} ],
			tagId : 'goodsCycleTypeEatslim3Day'
		});
		$("input[name=goodsCycleTypeEatslim3Day]").each(function() {
//			HGRM-8542 잇슬림 격일패턴 제거 - 체크되지 않도록 수정
//			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleTypeEatslim3Day, events: {change: onGoodsCycleTypeEatslim3DayChange}");
			$(this).attr("data-bind", "checked: false, events: {change: onGoodsCycleTypeEatslim3DayChange}");
		});

		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단(잇슬림) > 주차
		fnTagMkChkBox({
			id : 'goodsCycleTermTypeEatslim3Day',
			data : [ {
				"CODE" : "GOODS_CYCLE_TERM_TP.WEEK1",
				"NAME" : "1주"
			}, {
			...
			tagId : 'goodsCycleTermTypeEatslim3Day'
		});
		$("input[name=goodsCycleTermTypeEatslim3Day]").each(function() {
//			HGRM-8542 잇슬림 격일패턴 제거 - 체크되지 않도록 수정
//			$(this).attr("data-bind", "checked: ilGoodsDetail.goodsCycleTermTypeEatslim3Day, events: {change: onGoodsCycleTermTypeEatslim3DayChange}");
			$(this).attr("data-bind", "checked: false, events: {change: onGoodsCycleTermTypeEatslim3DayChange}"); // check 되지 않도록 강제 세팅
		});
...
//			max: lastDate,
	...
	function fnSaleType(supplierCode, mallinmallCategoryId){

		var goodsDailyTypeData = null;

		if(supplierCode == "FD") {		//공급처가 풀무원 FDD이면
			goodsDailyTypeData = [ {
				"CODE" : "GOODS_DAILY_TP.GREENJUICE",
				"NAME" : "일일판매(FDD_풀무원 녹즙)"
			} ]

			$("#goodsDailyTypeTh").attr("rowspan", "1");
			viewModel.ilGoodsDetail.set("isGoodsDailyType", false);

			viewModel.ilGoodsDetail.set("goodsDailyAllergyYn", "");
			viewModel.ilGoodsDetail.set("goodsCycleType7Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTermType7Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleType6Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTermType6Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleType3Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTermType3Day", []);
			viewModel.ilGoodsDetail.set("goodsDailyBulkYn", "N");
			viewModel.ilGoodsDetail.set("goodsBulkType", []);
			...
			fnTagMkChkBox({
				id : 'greenJuiceCleanseOpt',
				data : [ {
					"CODE" : "Y",
					"NAME" : "녹즙_클렌즈 옵션사용 [배송주기 : 주1회(월/목/금) & 배송기간 : 4주,8주]"
				} ],
				tagId : 'greenJuiceCleanseOpt'
			});

			//판매 정보 > 판매유형 > 속성 추가
			$("input[name=greenJuiceCleanseOpt]").each(function() {
				$(this).attr("data-bind", "checked: ilGoodsDetail.greenJuiceCleanseOpt");
			});

			$("input[name=greenJuiceCleanseOpt]:eq(0)").prop('checked', viewModel.ilGoodsDetail.greenJuiceCleanseOpt);
		}
		else if(supplierCode == "DM" && mallinmallCategoryId == "MALL_DIV.BABYMEAL") {		//공급처가 풀무원 PDM이고 브랜드가 베이비밀 이면
			goodsDailyTypeData = [ {
				"CODE" : "GOODS_DAILY_TP.BABYMEAL",
				"NAME" : "일일판매(PDM_베이비밀)"
			} ]

			$("#goodsDailyTypeTh").attr("rowspan", "2");
			viewModel.ilGoodsDetail.set("isGoodsDailyType", true);
			viewModel.ilGoodsDetail.set("babyMealVisible", true);
			viewModel.ilGoodsDetail.set("goodsCycleType5Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTermType5Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTypeEatslim3Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTermTypeEatslim3Day", []);
		}
		else if(supplierCode == "DM" && mallinmallCategoryId == "MALL_DIV.EATSLIM") {		//공급처가 풀무원 PDM이고 브랜드가 잇슬림 이면
			goodsDailyTypeData = [ {
				"CODE" : "GOODS_DAILY_TP.EATSSLIM",
				"NAME" : "일일판매(PDM_잇슬림)"
			} ]

			$("#goodsDailyTypeTh").attr("rowspan", "2");
			viewModel.ilGoodsDetail.set("isGoodsDailyType", true);
			viewModel.ilGoodsDetail.set("eatSlimVisible", true);
			viewModel.ilGoodsDetail.set("goodsDailyAllergyYn", "");
			viewModel.ilGoodsDetail.set("goodsCycleType7Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleTermType7Day", []);
			viewModel.ilGoodsDetail.set("goodsCycleType6Day", []);
		...
		//console.log("goodsDailyTypeData : ", goodsDailyTypeData);

		if(goodsDailyTypeData != null && goodsDailyTypeData != ""){
			viewModel.ilGoodsDetail.set("goodsDailyType", goodsDailyTypeData[0].CODE);

			//판매 정보 > 판매유형
			fnTagMkRadio({
				id : 'goodsDailyType',
				data : goodsDailyTypeData,
				tagId : 'goodsDailyType',
				chkVal : goodsDailyTypeData[0].CODE
			});
		}
	}

	//가격 정보 > 판매 가격정보 > 변경이력 보기
	function fnGoodsPricePopup(discountTypeCode, discountTypeCodeName) {
		if(ilGoodsId != ""){
			let params = {};
			params.ilGoodsId = ilGoodsId;
			params.priceKind = "price";
	...
	// 식단 정보 > 식단스케쥴 > 식단스케쥴 관리 화면 호출
	function fnMealSchedulePopup(patternCd){

        window.open("#/mealScheduleList");
    }

    // 식단 정보 > 식단스케쥴 > 패턴 상세보기
    function fnMealPatternDetailPopup(patternCd){
		...
		if( data.rtnValid ){
			fnKendoMessage({
				...
				ok : function() {
					...
					// 녹즙_클렌즈 옵션사용 체크여부 확인
					if($("input:checkbox[name=greenJuiceCleanseOpt]").is(":checked") == true) {
						viewModel.ilGoodsDetail.set("greenJuiceCleanseOpt", true);
					}
					else{
						viewModel.ilGoodsDetail.set("greenJuiceCleanseOpt", false);
					}
					...
					var addGoodsParam = {
						...
						*/
						,	promotionNameStartDate					: fnNvl(kendo.toString(viewModel.fnGetDateFull("promotionNameStart"), "yyyy-MM-dd HH:mm"))		// 프로모션 시작일
						,	promotionNameEndDate					: fnNvl(kendo.toString(viewModel.fnGetDateFull("promotionNameEnd"), "yyyy-MM-dd HH:mm"))		// 프로모션 종료일
						...
						*/
						...
						/* 판매정보 시작*/
						,	saleType								: "SALE_TYPE.DAILY"													//판매유형
						,	goodsDailyType							: viewModel.ilGoodsDetail.get("goodsDailyType")						//일일상품유형
						,	goodsDailyAllergyYn						: viewModel.ilGoodsDetail.get("goodsDailyAllergyYn")				//판매유형 > 일일 판매 옵션 설정 > 알러지 식단 포함여부
						,	goodsCycleType7Day						: viewModel.ilGoodsDetail.get("goodsCycleType7Day")					//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단
						,	goodsCycleTermType7Day					: viewModel.ilGoodsDetail.get("goodsCycleTermType7Day")				//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주7일 식단 > 주차
						,	goodsCycleType6Day						: viewModel.ilGoodsDetail.get("goodsCycleType6Day")					//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단
						,	goodsCycleTermType6Day					: viewModel.ilGoodsDetail.get("goodsCycleTermType6Day")				//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주6일 식단 > 주차
						,	goodsCycleType5Day						: viewModel.ilGoodsDetail.get("goodsCycleType5Day")					//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단
						,	goodsCycleTermType5Day					: viewModel.ilGoodsDetail.get("goodsCycleTermType5Day")				//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주5일 식단 > 주차
						,	goodsCycleType3Day						: viewModel.ilGoodsDetail.get("goodsCycleType3Day")					//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단
						,	goodsCycleTermType3Day					: viewModel.ilGoodsDetail.get("goodsCycleTermType3Day")				//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
						,	goodsCycleTypeEatslim3Day				: viewModel.ilGoodsDetail.get("goodsCycleTypeEatslim3Day")			//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단(잇슬림)
						,	goodsCycleTermTypeEatslim3Day			: viewModel.ilGoodsDetail.get("goodsCycleTermTypeEatslim3Day")		//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단(잇슬림) > 주차
						,	goodsDailyBulkYn						: viewModel.ilGoodsDetail.get("goodsDailyBulkYn")					//판매 정보 > 일일 판매 옵션 설정 > 일괄 배달 설정 > 허용 여부
						,	goodsBulkType							: viewModel.ilGoodsDetail.get("goodsBulkType")						//판매 정보 > 일일 판매 옵션 설정 > 식단주기 선택 > 주3일 식단 > 주차
						,	greenJuiceCleanseOpt					: viewModel.ilGoodsDetail.get("greenJuiceCleanseOpt")				//판매 정보 > 녹즙_클렌즈 옵션 사용
						/* 판매정보 끝*/
						...
						/* HGRM-1646 초기화 및 시간달력포맷 공통화 수정
						,	noticeBelow1StartDate					: kendo.toString(viewModel.ilGoodsDetail.get("noticeBelow1StartDate"),"yyyy-MM-dd")	//상세 상단공지 시작일
						,	noticeBelow1EndDate						: kendo.toString(viewModel.ilGoodsDetail.get("noticeBelow1EndDate"),"yyyy-MM-dd")	//상세 상단공지 종료일
						*/
						...
						/* HGRM-1646 초기화 및 시간달력포맷 공통화 수정
						,	noticeBelow2StartDate					: kendo.toString(viewModel.ilGoodsDetail.get("noticeBelow2StartDate"),"yyyy-MM-dd")	//상세 하단공지 시작일
						,	noticeBelow2EndDate						: kendo.toString(viewModel.ilGoodsDetail.get("noticeBelow2EndDate"),"yyyy-MM-dd")	//상세 하단공지 종료일
						*/
						...
						success : function(data) {
							...
							fnKendoMessage({
								...
								ok : function() {
									...
									if(pageMode == "create") {
										var successUrl = "/layout.html#/goodsDaily?ilGoodsId="+data['ilGoodsId'];
										location.href = successUrl;
									}
									else{
...
*/
	...
	$scope.fnGoodsSearchPopup = function( ){		//혜택/구매정보 > 추가상품 > 추가상품 검색 팝업 호출
		fnGoodsSearchPopup();
	};

	$scope.fnGoodsPricePopup = function(discountTypeCode, discountTypeCodeName){		//가격 정보 > 판매 가격정보 > 변경이력 보기
		fnGoodsPricePopup(discountTypeCode, discountTypeCodeName);
	};
    ...
    // 식단 스케쥴 관리 팝업
	$scope.fnMealSchedulePopup = function(){									        //식단 정보 > 식단 스케쥴 > 식단 스케쥴 관리 팝업
        fnMealSchedulePopup();
    };

    // 식단 스케쥴 관리 팝업
    $scope.fnMealPatternDetailPopup = function(patternCd){								        //식단 정보 > 식단 스케쥴 > 패턴 상세보기
        fnMealPatternDetailPopup(patternCd);
    };
