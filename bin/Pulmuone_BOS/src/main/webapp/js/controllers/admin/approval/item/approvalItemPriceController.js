/**-----------------------------------------------------------------------------
 * description : 품목가격 승인목록 처리
 * @
 * @ 아래 품목가격 공통 처리
 * @  - 품목가격 승인요청 내역 : approvalItemPriceRequestController.js, approvalItemPriceRequest.html
 * @  - 품목가격 승인관리 : approvalItemPriceAcceptController.js  , approvalItemPriceAccept.html
 * @
 * @ 수정일			수정자		수정내용
 * @ ---------------------------------------------------------------------------
 * @ 2021.02.22		박승현		최초생성
...
$(document).ready(function() {
	...
	function fnInitialize(){
	    ...
	    // ------------------------------------------------------------------------
	    // 품목가격 승인 요청 목록, 승인 대상 목록 Set
	    //   - approvalItemPriceRequestController.js 에서 설정 됨
		//   - approvalItemPriceAcceptController.js 에서 설정 됨
	    // ------------------------------------------------------------------------
	    ...
	    // ------------------------------------------------------------------------
	    if(!fnIsEmpty(pageId)) {
	    	if (pageId == 'approvalItemPriceRequest') {
	    		$('#pageTitleSpan').text('품목가격 승인요청 내역');
	    	}else if (pageId == 'approvalItemPriceAccept') {
	    		$('#pageTitleSpan').text('품목가격 승인관리');
	    	}
	    }
...
//		$('#searchForm').formClear(true);
	...
	function fnDefaultSet(){
		...
		if(!(PG_SESSION == null || PG_SESSION == '' || PG_SESSION == undefined || PG_SESSION == 'undefined')) {
			...
			if(pageId == 'approvalItemPriceAccept')
...
//            cscdField  : "warehouseGroupCode"
        ...
        fnTagMkChkBox({
	        ...
	        //,afterData : [{ "CODE" : "APPRAUTO", "NAME" : "승인완료(시스템)" },{ "CODE" : "DISPOSAL", "NAME" : "폐기" }]
	    });
	...
	function fnSearch() {
		...
		if($("input[name=selectConditionType]:checked").val() == "singleSection") {
			...
			if(pageId == 'approvalItemPriceRequest')
				data['searchApprovalStatus'] = 'APPR_STAT.REQUEST∀APPR_STAT.CANCEL∀APPR_STAT.DENIED∀APPR_STAT.SUB_APPROVED∀APPR_STAT.APPROVED∀APPR_STAT.APPROVED_BY_SYSTEM';
			else if(pageId == 'approvalItemPriceAccept')
				data['searchApprovalStatus'] = 'APPR_STAT.REQUEST∀APPR_STAT.DENIED∀APPR_STAT.SUB_APPROVED∀APPR_STAT.APPROVED∀APPR_STAT.APPROVED_BY_SYSTEM';
		}else {
	...
	function fnInitGrid() {
		...
		aGridDs = fnGetPagingDataSource({
			url : '/admin/approval/item/getApprovalItemPriceList',
			pageSize : PAGE_SIZE,
				...
				, columns : [
						{ field : 'chk', headerTemplate : "<input type='checkbox' id='checkBoxAll' />", width:'50px', attributes : {style : "text-align:center;"}
							, template : function(dataItem){
									...
									if (dataItem.apprStat == 'APPR_STAT.REQUEST') {
										if (!fnIsEmpty(dataItem.approvalSubUserId)) { // 1차 승인자 설정시, 1차 승인자(권한대행 포함)에게만 노출
											if (
												...
												returnValue = '<input type="checkbox" name="itemGridChk" class="itemGridChk" value="' + dataItem.ilItemPriceApprId + '" />';
										}
								...
								else if (pageId == 'approvalItemPriceRequest') {
									if (dataItem.apprStat == 'APPR_STAT.REQUEST') {
										if (
												PG_SESSION != undefined && PG_SESSION != null && PG_SESSION.loginId != undefined && PG_SESSION.loginId != null
						...
						, {field : 'ilItemBarcode',  title : '품목코드<br>품목바코드', width : '140px', attributes : { style : 'text-align:center;color:blue;text-decoration:underline' }
							, template: function(dataItem) {
								if(dataItem['ilItemCode'] != "" && dataItem['ilItemBarcode'] != "") {
									...
									return dataItem['ilItemCode'] + "<br>" + "-";
								}else if(dataItem['ilItemCode'] == "" && dataItem['ilItemBarcode'] != "") {
									return  "-" + "<br>" + dataItem['ilItemBarcode'];
								}else if( dataItem['ilItemCode'] == "" && dataItem['ilItemBarcode'] == "" ) {
									return "-" + "<br>" + "-";
								}
							}
						}
						, {field : 'itemTypeName', title : '품목유형', width : '80px', attributes : { style : 'text-align:center' } }
						, {field : 'erpLinkIfYn', title : 'ERP 연동여부', width : '80px', attributes : { style : 'text-align:center' }
							, template: function(dataItem) {
								return dataItem['erpLinkIfYn'] == true ? '연동' : '미연동';
							}
						}
						, {field : 'itemName', title : '품목명', width : '200px', attributes : { style : 'text-align:center;color:blue;text-decoration:underline' } }
						...
						, {field : 'approvalRequestUserName', title : '승인요청자', width : '100px', attributes : {style : 'text-align:center'},
							template : function(dataItem) {
						...
						, {field : 'approvalSubUserName', title : '1차 승인관리자<BR>/권한위임자', width : '100px', attributes : {"class" : "#=(!fnIsEmpty(approvalSubChangeDt))?'bgColorGreen':''#",style : 'text-align:center'},
							template : function(dataItem) {
								var templateString = "";
						...
						, {field : 'approvalUserName', title : '최종승인관리자<BR>/권한위임자', width : '100px', attributes : {"class" : "#=(!fnIsEmpty(approvalChangeDt))?'bgColorGreen':''#", style : 'text-align:center'},
							template : function(dataItem) {
								var templateString = "";
						...
						, {field : 'apprStatName', title : '승인상태', width : '100px', attributes : {style : 'text-align:center'},
							template : function(dataItem) {
						...
						, {title : '관리', width: "180px", attributes:{ style:'text-align:left;', class:'forbiz-cell-readonly' }
							,command: [ { text: '요청철회', imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon"
											, click: function(e) {
												e.preventDefault();
												var tr = $(e.target).closest("tr");
												var data = this.dataItem(tr);
												fnCancelRequestOne(data.ilItemPriceApprId);
											}
										...
										, { text: '삭제', imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon"
											, click: function(e) {
												...
												var tr = $(e.target).closest("tr");
												var data = this.dataItem(tr);
												fnDisposal(data.ilItemPriceApprId);
											}
											, visible: function(dataItem) {
												return ( pageId == 'approvalItemPriceRequest'
										...
										, { text: '승인완료'		, imageClass: "k-i-add", className: "f-grid-add k-margin5", iconClass: "k-icon",
											click: function(e) {  e.preventDefault();
																	var tr = $(e.target).closest("tr");
																	var data = this.dataItem(tr);
																	fnConfirmApproval(data.ilItemPriceApprId);}
											,visible: function(dataItem) {
												var hasAuth = false;

												if (pageId != 'approvalItemPriceAccept') // 승인관리 페이지에서만
													return false;
										...
										, { text: '승인반려' , imageClass: "k-i-add", className: "f-grid-search k-margin5", iconClass: "k-icon",
											click: function(e) {  e.preventDefault();
																	var tr = $(e.target).closest("tr");
																	var data = this.dataItem(tr);
																	fnDenieApprovalPopup(data.ilItemPriceApprId);
												            	}
											, visible: function(dataItem) {
												var hasAuth = false;
						...
						, {field : 'ilItemPriceApprId' , title : '품목가격 승인 PK' , attributes : {style : 'text-align:center'}, hidden:true}
					]
		};
...
//			if(colIdx == 16){
//			    if("APPR_STAT.DENIED" == dataItem.apprStat){
//                    fnGetDeniedInfo(dataItem);
//			    }
//			}
		});
	};
	...
	function fnGetSelectType(ilItemPriceApprId) {
		if (ilItemPriceApprId == undefined) {
			viewModel.popupSelectType = SELECT_TYPE.CHECKBOX;
		} else {
			viewModel.ilItemPriceApprId = ilItemPriceApprId;
	...
	//승인 반려사유 팝업
	function fnDenieApprovalPopup(ilItemPriceApprId){
		fnKendoInputPoup({height:"400px" ,width:"500px", title:{ nullMsg :'승인 반려 사유' } });
		fnGetSelectType(ilItemPriceApprId);
	}

	//승인처리
	function fnConfirmApproval(ilItemPriceApprId) {
		let params = {};
		let selectType = fnGetSelectType(ilItemPriceApprId);
		...
		if (SELECT_TYPE.BUTTON == selectType) {
			params.ilItemPriceApprIdList[0] = ilItemPriceApprId;
		}
		...
		if (SELECT_TYPE.CHECKBOX == selectType) {
			...
			for(let i = 0, selectCount = selectRows.length; i < selectCount; i++){
				...
				params.ilItemPriceApprIdList[i] = dataItem.ilItemPriceApprId;
			}
		}

		if( params.ilItemPriceApprIdList.length == 0 ){
	...
	function fnDenieApproval() {
		...
		if (SELECT_TYPE.BUTTON == selectType) {
			params.ilItemPriceApprIdList[0] = viewModel.ilItemPriceApprId;
		}
    ...
    function fnApprovalProcess(params){
    	if( fnIsEmpty(params) || fnIsEmpty(params.ilItemPriceApprIdList) || params.ilItemPriceApprIdList.length < 1 || fnIsEmpty(params.apprStat)){
    	...
    	fnKendoMessage({
			...
			ok      : function(){
		    	fnAjax({
		    		url     : "/admin/approval/item/putApprovalProcessItemPrice",
		    		params  : params,
		    		contentType : "application/json",
		    		success : function( data ){
		    		    var resultMessage = "상태값 변경이 완료되었습니다.(승인처리 불가 목록 제외)";
		                if(data.failCount > 0){
		                    resultMessage = "시스템 오류를 제외한 상태값 변경이 완료되었습니다.(" + failMessageCode + ")";
		                }
    ...
    function fnCancelRequestOne(ilItemPriceApprId){
		if( ilItemPriceApprId.length == 0 ){
			fnKendoMessage({ message : "선택된 승인건이 없습니다." });
			return;
		}
		let params = {};
	...
	function fnSelectCancelRequest(){
		...
		if( selectRows.length == 0 ){
			fnKendoMessage({ message : "선택된 정보가 없거나 변경할 수 있는 목록이 존재하지 않습니다." });
			return;
		}
		for(let i = 0, selectCount = selectRows.length; i < selectCount; i++){
			let dataItem = aGrid.dataItem($(selectRows[i]));
	...
	function fnCancelRequest(params){
		if( fnIsEmpty(params) || fnIsEmpty(params.ilItemPriceApprIdList) || params.ilItemPriceApprIdList.length < 1){
			fnKendoMessage({ message : "선택된 승인건이 없습니다." });
			return;
		}
	...
	//폐기
	function fnDisposal(ilItemPriceApprId){
    	if( ilItemPriceApprId.length == 0 ){
    		fnKendoMessage({ message : "선택된 승인건이 없습니다." });
    		return;
    	}
    	...
    	fnKendoMessage({
			...
			ok      : function(){
		    	fnAjax({
		    		url     : "/admin/approval/item/putDisposalApprovalItemPrice",
					params  : params,
					contentType : "application/json",
					success : function( data ){
    ...
    //반려사유 조회
    function fnGetDeniedInfo(data){
        let params = {};
        params.ilItemPriceApprIdList = [];
        params.ilItemPriceApprIdList[0] = data.ilItemPriceApprId;

        fnAjax({
            url     : "/admin/approval/item/getApprovalDeniedInfo",
            params  : params,
            contentType : "application/json",
            success : function( data ){
                fnKendoMessage({message : data, ok : function(){}});
            },
            isAction : "search"
        });
    }
	...
	$scope.fnDenieApprovalPopup = function() {
	    fnKendoMessage({
            message:'이미 처리되었거나, 처리불가 목록을 제외한 선택목록이 승인반려 처리됩니다.<BR>진행하시겠습니까?'
            ,type : "confirm"
            ,ok : function(){
	...
	$scope.fnConfirmApprovalChkbox = function() {
        fnKendoMessage({
            message:'이미 처리되었거나, 처리불가 목록을 제외한 선택목록이 승인완료됩니다.<BR>진행하시겠습니까?'
            ,type : "confirm"
            ,ok : function(){
                fnConfirmApproval();
            }
