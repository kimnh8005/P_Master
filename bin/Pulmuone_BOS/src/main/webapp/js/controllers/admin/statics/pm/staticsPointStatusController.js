/**-----------------------------------------------------------------------------
 * description 		 : 통계관리 > 프로모션통계 > 적립금 현황 통계
 * @
 * @ 수정일			수정자          수정내용
 * @ ------------------------------------------------------
 * @ 2021.07.23		안치열          최초생성
...
$(document).ready(function() {
	...
	function fnSearch(){
		...
		if(data.findYear == "" || data.findMonth == ""){
		    fnKendoMessage({message : '월/연도를 입력해주세요.'});
		    return;
		}
	...
	function fnInitOptionBox(){
        ...
        fnKendoDropDownList({
          ...
          , tagId   : 'searchOrganization'
          // , chkVal  : 'singleSection'
          , tab     : true
          , data    : [ {CODE : 'ONLINE' , NAME  : '온라인사업부'}
                      , {CODE : 'ETC' , NAME  : '온라인사업부 외'}
                      ]
          , chkVal  : ''
          , blank   : "전체"
        });
	...
	function fnInitGrid(){
		gridDs = fnGetDataSource({
			url      : "/admin/statics/pm/getStaticsPointStatusList"
		});
		gridOpt = {
			dataSource: gridDs
            ...
            , autobind    : false
			,columns   : [
                  { field:'issueDeptName' ,title : '분담조직'   	,width:'300px' ,attributes:{ style:'text-align:center' }}
                , { field:'pointType'     ,title : '적립금 유형'	,width:'300px' ,attributes:{ style:'text-align:center' }}
                , { field:'statics'       ,title : '금액내역'   	,width:'300px' ,attributes:{ style:'text-align:center' }}
                , { field:'amount'        ,title : '합계'	    ,width:'300px' ,attributes:{ style:'text-align:right' }, format: '{0:\#\#,\#}'}
            ]
		};
		...
		grid.bind("dataBound", function() {
			...
			// rowspan
            mergeGridRows('grid', ['issueDeptName', 'pointType'], ['issueDeptName']);
    ...
    $scope.fnExcelDownload = function() {
        ...
        fnExcelDownload('/admin/statics/pm/getExportExcelStaticsPointStatusList', data);
    };
