...
public enum BosStorageInfoEnum implements StorageInfoBaseType {

    /*
     * 도메인별 파일 업로드시 저장 경로 정보
     */
    UR_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.UR, //
            "ur", SubDateTimePath.YEAR_TO_DATE //
            , UploadOptionInfoDto.builder().build() //
    ) //
    , IL_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.IL, //
            "il", SubDateTimePath.YEAR_TO_DATE //
            , UploadOptionInfoDto.builder().build() //
    ) //
    , SN_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.SN, //
            "sn", SubDateTimePath.YEAR_TO_DATE //
            , UploadOptionInfoDto.builder().build() //
    ) //
    , CS_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.CS, //
            "cs", SubDateTimePath.YEAR_TO_DATE //
            , UploadOptionInfoDto.builder().build() //
    ) //
    , DP_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.DP, //
            "dp", SubDateTimePath.YEAR_TO_DATE //
            , UploadOptionInfoDto.builder().build() //
        ) //
    , QR_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.QR, //
    		"dp/qr", SubDateTimePath.YEAR_TO_DATE //
    		, UploadOptionInfoDto.builder().build() //
    		) //
    , ST_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.ST, //
    		"st", SubDateTimePath.YEAR_TO_DATE //
    		, UploadOptionInfoDto.builder().build() //
    		) //
    , PS_PUBLIC_STORAGE_INFO(StorageType.PUBLIC, BosDomainPrefixEnum.PS, //
            "ps", SubDateTimePath.YEAR_TO_DATE //
            , UploadOptionInfoDto.builder().build() //
    ) //
    ;
    ...
    private BosDomainPrefixEnum domainPrefix; // 관련된 도메인 prefix
    ...
    BosStorageInfoEnum(StorageType storageType, BosDomainPrefixEnum domainPrefix, String subStoragePath, SubDateTimePath subDateTimePath, UploadOptionInfoDto uploadOptionInfo) {
    ...
    public static String getRootStoragePath(String storageType) {
        ...
        if (storageType.equalsIgnoreCase(StorageType.PUBLIC.name())) {
            return BosStorageInfoEnum.publicRootLocation;
        }
    ...
    public static String getFullSubStoragePath(String storageType, String domainPrefix) {
        ...
        for (BosStorageInfoEnum storageInfoEnum : BosStorageInfoEnum.values()) {
    ...
    public BosDomainPrefixEnum getDomainPrefix() {
    ...
    static class StorageRootLocationInjector implements InitializingBean {
        ...
        public void afterPropertiesSet() throws Exception {
            ...
            BosStorageInfoEnum.moduleName = moduleNameValue;

            // 프로파일별로 public 저장소 경로를 다르게 세팅 : SystemUtil.getAbsolutePathByProfile(String path) 메서드 참조
            BosStorageInfoEnum.publicRootLocation = SystemUtil.getAbsolutePathByProfile(publicRootLocation);
        }
