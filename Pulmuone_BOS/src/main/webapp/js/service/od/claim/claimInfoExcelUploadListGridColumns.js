...
var ifDayUploadListGridUtil = {
		...
		return [
				...
				,{ field:'batchEnd'   ,title : '업로드 Batch 종료일자<BR>(소요시간)'	, width:'90px',attributes:{ style:'text-align:center' },
	                template : function(dataItem){
				...
				,{ field:'admin'	,title : '관리자'	, width:'90px',attributes:{ style:'text-align:center' },
	                template : function(dataItem){
	                    return dataItem.createName + " / " + dataItem.loginId;
	                }
	            }
	            ,{ field:'createDateName' ,title : '등록일자',width:'90px',attributes:{ style:'text-align:center' }}
				,{ field:'uploadJsonData' ,title : '업로드파일',width:'100px',attributes:{ style:'text-align:center' },
					template : function(dataItem){
						let rtnValue = "";
						if(fnNvl(dataItem.uploadJsonData) != "") {
				...
				,{ field:'management'		    ,title : '관리'		, width:'200px', attributes : { style : "text-align:center" },
					template : function(dataItem){
						...
						if(dataItem.batchStatusCode == '22' && (dataItem.successCount - dataItem.orderCount) > 0) { // 배치상태 완료 이며 실패내역이 존재 할 경우
							if (fnIsProgramAuth("EXCELDOWN")) {
	...
	claimUploadList : function() {
		return [
			{ field:'no'                   ,title : 'No'	, width:'50px',attributes:{ style:'text-align:center' }, template: "<span class='row-number'></span>"}
			,{ field:'successCount'			,title : '업로드'	    , width:'90px',attributes:{ style:'text-align:center' },
				template : function(dataItem){
					return "성공 : " + dataItem.successCount + " 건 / 실패 : " + dataItem.failCount + " 건";
				}
			}
			,{ field:'failCount'			,title : '업데이트'		, width:'90px',attributes:{ style:'text-align:center' },
				template : function(dataItem){
					if(dataItem.batchStatusCode == '22') {
						return "성공 : " + dataItem.claimCreateCnt + " 건 / 실패 : " + (dataItem.successCount - dataItem.claimCreateCnt) + " 건";
					}
					else {
						return "-";
					}
			...
			,{ field:'management'		    ,title : '관리'		, width:'150px', attributes : { style : "text-align:center" },
				template : function(dataItem){
					...
					if(dataItem.batchStatusCode == '22' && (dataItem.successCount - dataItem.claimCreateCnt) > 0) { // 배치상태 완료 이며 실패내역이 존재 할 경우
						if (fnIsProgramAuth("EXCELDOWN")) {
							rtnValue += '&nbsp;&nbsp;<button type="button" class="k-button k-button-icontext btn-s k-grid-실패내역다운로드" kind="btnExcelDown" data-failtype="B">업데이트 실패내역</button>';
						}
					} else {
