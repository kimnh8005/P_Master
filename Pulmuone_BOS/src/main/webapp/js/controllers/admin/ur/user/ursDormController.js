...
$(document).ready(function() {
	...
	function fnSave(e){
		...
		fnKendoMessage( {message:fnGetLangData({key :"",nullMsg :'휴면해제 하시겠습니까?' })
		,type	: "confirm"
		,ok		:
			function(e){
				var url  = '/admin/ur/UserDormant/putUserDormant';
				var cbId= 'update';
				fnAjax({
					url     : url,
					params  : {"urUserId": dataItem.urUserId,"urUserMoveId" : dataItem.urUserMoveId},
					success :
						function( data ){
	...
	function fnDefaultSet(){
	    $('#searchOne').show(); //단일조건 show
	    $('#searchMany').hide(); //복수조건 hide
	...
	function fnInitGrid(){
		aGridDs = fnGetPagingDataSource({
			url      : '/admin/ur/UserDormant/getUserDormantList'
			,pageSize : PAGE_SIZE
		});
		aGridOpt = {
			dataSource: aGridDs
			...
			,columns   : [
					...
					,{ field:'createDate'		,title : '휴면일'		, width:'90px'	,attributes:{ style:'text-align:center' }}
					,{ command: { text: "정상전환", click: fnSave, className:'btn-white btn-s' }, title: '관리', width: "80px", attributes:{ style:'text-align:center'}}
					,{ field:'urUserId'			,hidden: true}
					,{ field:'urUserMoveId'		,hidden: true}
			]
		};
		...
		$("#aGrid").on("click", "tbody>tr>td", function () {
			...
			if($(this).closest('table').find('th').eq(index).text() == '회원명' || $(this).closest('table').find('th').eq(index).text() == '회원ID'){
				...
				fnKendoPopup({
		            ...
		            param: { "urUserId": map.urUserId,"type": "userDorm" },
		            src: '#/buyerPopup',
		            width: '1200px',
		            height: '700px',
		...
		$("#checkBoxAll").on("click",function(index){
			if($("#checkBoxAll").prop("checked")==true){
				...
				$('#fnSave').data('kendoButton').enable(true);
			}else{
				$('INPUT[name=itemGridChk]').prop("checked",false);
				$('#fnSave').data('kendoButton').enable(false);
			}
		});
