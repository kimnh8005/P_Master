...
$(document).ready(function() {
	...
	function fnDuplicateGoods(){
		...
		let paramGoodsType = "GOODS_TYPE.RENTAL";
			...
			fnAjax({
				...
				success	: function(data){
					...
					if(data.ilGoodsId){
						...
						fnInitGoodsRental();			// Kendo Component 초기화
						init();
	...
	function fnViewModelSetting(){
		...
		viewModel = new kendo.data.ObservableObject({															//Kendo UI MVVM Pattern 적용
			...
			ilGoodsDetail : {
				...
				goodsType							: 'GOODS_TYPE.RENTAL',										// 상품 유형(goodsRental 페이지는 무조건 렌탈상품 페이지 이므로 고정값으로 저장)
				goodsTypeName						: '렌탈',													// 코드에 따라서 불러오도록 변경 해야 함
				ilGoodsId							: '',														// 상품코드
		...
		fnAjax({
			...
			success	:
				function( data ){
					...
					//가격 정보 시작(렌탈상품만 품목에서 가격정보 가져옴)
					$("#rentalFeePerMonth").html(fnNumberWithCommas(data.row.rentalFeePerMonth) + " 원");
					$("#rentalDueMonth").html(data.row.rentalDueMonth + "개월");
					$("#allRentalFee").html(fnNumberWithCommas(data.row.rentalFeePerMonth*data.row.rentalDueMonth) + " 원 ( 렌탈료 X 의무사용기간 )");

					if (data.row.rentalDeposit != null && data.row.rentalDeposit != '')
						$("#rentalDeposit").html(fnNumberWithCommas(data.row.rentalDeposit) + " 원");

					if (data.row.rentalInstallFee != null && data.row.rentalInstallFee != '')
						$("#rentalInstallFee").html(fnNumberWithCommas(data.row.rentalInstallFee) + " 원");

					if (data.row.rentalRegistFee != null && data.row.rentalRegistFee != '')
						$("#rentalRegistFee").html(fnNumberWithCommas(data.row.rentalRegistFee) + " 원");
					//가격 정보 끝
						...
						success : function(data, result) {
							fnKendoMessage({
								...
								ok : function() {
									...
									if(pageMode == "create") {
										var successUrl = "/layout.html#/goodsRental?ilGoodsId="+data['ilGoodsId'];
										location.href = successUrl;
									}
									else{
