/*******************************************************************************************************************************************************************************************************
 * --------------- description : 상품 관리 - 폐기임박상품 등록/수정 @ ----------------------------
 ******************************************************************************************************************************************************************************************************/
'use strict';
...
var presentationEventGridDs, presentationEventGridOpt, presentationEventGrid;		//혜택/구매정보 > 증정 행사 Grid
...
$(document).ready(function() {
	...
	function fnDuplicateGoods(){
		...
		let paramGoodsType = "GOODS_TYPE.DISPOSAL";
			...
			fnAjax({
				...
				success	: function(data){
					if(data.ilGoodsId){
						...
						fnPresentationEventGrid();	// 혜택/구매정보 > 증정 행사 Grid > 추후 kendo template로 대체 처리할 예정
	...
	function fnDefaultValue() {
		if(ilGoodsId != undefined && ilGoodsId != ""){	//상품 수정이라면
			...
			viewModel.ilGoodsDetail.set("goodsType", "GOODS_TYPE.DISPOSAL");		// 상품 유형
			viewModel.ilGoodsDetail.set("goodsTypeName", "폐기임박");				// 상품 유형명

			viewModel.ilGoodsDetail.set("saleStatus", 'SALE_STATUS.SAVE');
			viewModel.ilGoodsDetail.set("goodsOutmallSaleStatus", 'GOODS_OUTMALL_SALE_STAT.NONE');
	...
	function fnViewModelSetting(){
		...
		viewModel = new kendo.data.ObservableObject({															//Kendo UI MVVM Pattern 적용
			...
			ilGoodsDetail : {
				...
				goodsType							: 'GOODS_TYPE.DISPOSAL',			// 상품 유형
				goodsTypeName						: '폐기임박',						// 코드에 따라서 불러오도록 변경 해야 함
				ilGoodsId							: '',								// 상품코드
				...
				/* 판매 정보 Start */
				saleType								: 'SALE_TYPE.NORMAL',	// 판매유형 > 일반판매(기본설정)
				...
				reviewReward								: '1',								//혜택 설정 > 적립금 지급설정 > 후기 적립 > 개별 설정(기본설정)
				reviewRewardRateKind						: '1',								//혜택 설정 > 적립금 지급설정 > 후기 적립 > 정률,정액 dropdownlist > 정액(기본설정)
				isReviewReward								: true,
			...
			//혜택 설정 > 적립금 지급설정 > 후기 적립 > 개별 설정일때, 정률, 정액 DropdownLIst disable = false;
			onReviewRewardChange				: function(e) {
				switch(viewModel.ilGoodsDetail.reviewReward) {
					case '1' :
						viewModel.ilGoodsDetail.set('isReviewReward', true);
						break;
					case '2' :
						viewModel.ilGoodsDetail.set('isReviewReward', false);
						break;
				}
			},
				...
				success	: function(data){
					if(data.ilGoodsDetail == null){
						...
						if(data.goodsPrice.length > 0){
							viewModel.set("isVisibleGoodsPriceInfo", true);
...
//			max: lastDate,
		...
		//혜택 설정 > 적립금 지급설정 > 후기 적립 > 속성 추가
		$("input[name=reviewReward]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.reviewReward, events: {change: onReviewRewardChange}");
		});

		//혜택 설정 > 적립금 지급설정 > 후기 적립 > 정률,정액
		fnKendoDropDownList({
			id : 'reviewRewardRateKind',
			data : [ {
				code : '1',
				name : '정률'
			}, {
		...
		//혜택 설정 > 적립금 지급설정 > 후기 적립 > 정률,정액 > 속성 추가
		$("input[name=reviewRewardRateKind]").each(function() {
			$(this).attr("data-bind", "value: ilGoodsDetail.reviewRewardRateKind, disabled: isReviewReward");
			$(this).attr("data-value-primitive", true);
		});
...
//			max: lastDate,
	...
	function fnPresentationEventGrid() {

		presentationEventGridDs = fnGetEditDataSource({
			url : '',
		});
		...
		presentationEventGridOpt = {
			dataSource : presentationEventGridDs,
			editable : false,
			columns : [
				{ field : "itemCode", title : "행사유형", width : "10%", attributes : {style : "text-align:center"} }
			,	{ field : "itemName", title : "증정행사 명", width : "50%", attributes : {style : "text-align:center"} }
			,	{ field : "itemName", title : "행사기간", width : "30%", attributes : {style : "text-align:center"} }
			,	{ field : "itemName", title : "상세보기", width : "20%", attributes : {style : "text-align:center"} }
			],
			noRecordMsg : '행사가 없습니다'
		};
		presentationEventGrid = $('#presentationEventGrid').initializeKendoGrid(presentationEventGridOpt).cKendoGrid();
	}
				...
				ok : function() {
					...
					var addGoodsParam = {
						...
						*/
						...
						,	saleShopYn								: viewModel.ilGoodsDetail.get("saleShopYn")										//매장판매
						...
						success : function(data) {
							...
							fnKendoMessage({
								...
								ok : function() {
									...
									if(pageMode == "create") {
										var successUrl = "/layout.html#/goodsDisposal?ilGoodsId="+data['ilGoodsId'];
										location.href = successUrl;
									}
									else{
