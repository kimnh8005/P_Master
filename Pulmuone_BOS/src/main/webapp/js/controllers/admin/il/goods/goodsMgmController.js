...
$(document).ready(function() {
	...
	function fnDuplicateGoods(){
		...
		let paramGoodsType = "GOODS_TYPE.NORMAL";
		...
		if(ilItemCode != undefined && ilItemCode != ""){	//상품 등록이라면
			fnAjax({
				...
				success	: function(data){
					//console.log("duplicateGoods data >>> " +  data.ilGoodsId);
					if(data.ilGoodsId){
			...
			fnAjax({
				...
				success	: function(data){
					if(data.ilGoodsId && data.goodsType != paramGoodsType){
						...
						fnDefaultValue();			// 등록 or 수정에 따른 기본 세팅
						setCacheData();
						fnGoodsAuthInputAllow();	// 본사, 거래처에 따른 input disabled 세팅
					}
				}
			});
		}
		//$("#inputForm input").attr("disabled", "disabled");
		//$("#inputForm button").attr("disabled", "disabled");
	}
	...
	function fnViewModelSetting(){
		...
		viewModel = new kendo.data.ObservableObject({															//Kendo UI MVVM Pattern 적용
			...
			ilGoodsDetail : {
				...
				goodsType							: 'GOODS_TYPE.NORMAL',										// 상품 유형(goodsMgm 페이지는 무조건 일반상품 페이지 이므로 고정값으로 저장)
				goodsTypeName						: '일반',													// 코드에 따라서 불러오도록 변경 해야 함
				ilGoodsId							: '',														// 상품코드
...
//				goodsReservationOptionList				: [],							// 판매유형(예약판매) > 예약 판매 옵션 설정 세부 항목 목록, goodsReservationOption-row-template 사용
				...
				saleShopYn								: 'N',							// 판매정보 > 판매유형 > 매장판매보
				presentYn								: 'Y',							// 판매정보 > 선물하기 허용
			...
			onSaleTypeChange 					: function(e) {
				if(e.target.value == "SALE_TYPE.NORMAL") {
					e.preventDefault();
			...
			fnReservationEndDateChange : function(e){ // 종료일 변경
				...
				if(dataItem.reservationStartDateOld <= nowDate ){
					// 기존 종료일 이후로 변경 못함.
					if(dataItem.ilGoodsReserveOptionId != null && dataItem.reservationEndDate >= dataItem.orderIfDate) {
						dataItem.reservationEndDate = dataItem.reservationEndDateOld;
						dataItem.reservationEndHour = dataItem.reservationEndHourOld;
						dataItem.reservationEndMinute = dataItem.reservationEndMinuteOld;
						viewModel.goodsReservationOptionList.sync();
						fnKendoMessage({ message : "예약 주문 가능 기간 종료일은 주문수집 I/F 일자 이후로 변경할 수 없습니다." });
						return;
					}
				}
			...
			fnAllOrderIfDateCalc : function(e) {	//주문수집I/F일자 전체 계산
				...
				if(viewModel.goodsReservationOptionList.total() > 0) {
					...
					for(var i=0; i <= viewModel.goodsReservationOptionList.total() - 1; i++){
						let dataItem = viewModel.goodsReservationOptionList.at(i);
						let reservationStartDate = dataItem.reservationStartDate;	// 예약주문 시작일
						let reservationEndDate = dataItem.reservationEndDate;		// 예약주문 종료일
						...
						success		: function(data, status, xhr){
							if(data) {
								for(var i in data){
									...
									// 전체 주문 I/F일자 재계산시 가능기간 시작일이 현재일보다 큰경우 조건 추가.
                                    if((dataItem.orderIfDateOld == '') || (dataItem.orderIfDateOld == data[i].orderIfDate) || dataItem.reservationStartDate > nowDate){
			...
			fnOrderIfDateCalc : function(e) {		//주문수집I/F 개별 일자 계산
				e.preventDefault();
			...
			fnAjax({
				...
				success	: function(data){
					if(data.ilGoodsDetail == null){
						...
						console.log("data.goodsDiscountPriorityList : ", data.goodsDiscountErpEventList);
						...
						// 선물하기 노출 여부 (미대상 NA)
						// ERP 연동 품목 && 공급업체: 올가홀푸드 && 대분류 채소 && 출고처의 BOS 발주유형 : R2 = 품목별 상이
						if(firstIlGoodsDetailData.erpIfYn == "Y" && firstIlGoodsDetailData.supplierCode == "OG" && firstIlGoodsDetailData.erpCategoryLevel1Id == "채소" && firstIlGoodsDetailData.poPerItemYn == "Y") {
							$("#presentYnTh").attr("hidden", true);
							$("#presentYnTd").attr("hidden", true);
							viewModel.ilGoodsDetail.set("presentYn", "NA");
						} else viewModel.ilGoodsDetail.set("presentYn", firstIlGoodsDetailData.presentYn);
...
//						fnApprHtml(data.ilGoodsDetail);
                    ...
                    if (data.ilGoodsDetail.goodsClientDispYn == true ||
                        data.ilGoodsDetail.godosClientSaleStartDt == true ||
                        data.ilGoodsDetail.godosClientSaleEndDt == true ||
                        data.ilGoodsDetail.goodsClientSaleStatus == true)$('#labelItemSaleDisplay').addClass('approvalChanged');  // 판매/전시 변경여부
					if (data.ilGoodsDetail.goodsClientDispYn == true)$('#labelItemDisplayYn').addClass('approvalChanged');        // 전시상태 변경여부
					if (data.ilGoodsDetail.godosClientSaleStartDt == true ||
					    data.ilGoodsDetail.godosClientSaleEndDt == true )$('#labelItemSaleDate').addClass('approvalChanged');     // 판매기간 변경여부
                    if (data.ilGoodsDetail.goodsClientSaleStatus == true)$('#itemLabelIsSaleStatus').addClass('approvalChanged'); // 판매상태 변경여부
					}
				}
			});
		}
		...
		fnAjax({
			...
			success	:
				function( data ){
					...
					// 선물하기 노출 여부 (미대상 NA)
					// ERP 연동 품목 && 공급업체: 올가홀푸드 && 대분류 채소 && 출고처의 BOS 발주유형 : R2 = 품목별 상이
					if(data.row.erpIfYn == "Y" && data.row.supplierCode == "OG" && data.row.erpCategoryLevel1Id == "채소" && data.row.poPerItemYn == "Y") {
						$("#presentYnTh").attr("hidden", true);
						$("#presentYnTd").attr("hidden", true);
						viewModel.ilGoodsDetail.set("presentYn", "NA");
					}
...
//			max: lastDate,
		...
		fnTagMkRadio({
			...
			data : [ {
				...
				"CODE" : "SALE_TYPE.REGULAR",
				"NAME" : "일반/정기판매"
			}, {
				"CODE" : "SALE_TYPE.RESERVATION",
				"NAME" : "예약판매"
			} ],
			tagId : 'saleType',
//			id : 'saleType',
...
//			}
		...
		//판매 정보 > 판매유형 > 매장판매 HGRM-2176 매장판매->매장 텍스트 변경
		fnTagMkChkBox({
			id : 'saleShopYn',
			data : [ {
				"CODE" : "Y",
		...
		//판매 정보 > 판매유형 > 속성 추가
		$("input[name=saleShopYn]").each(function() {
			$(this).attr("data-bind", "checked: ilGoodsDetail.saleShopYn");
			$(this).attr("disabled", true);
				...
				ok : function() {
					...
					var addGoodsParam = {
						...
						*/
						...
						,	presentYn								: viewModel.ilGoodsDetail.get("presentYn")										//선물하기 허용
					...
					fnAjax({
						...
						success : function(data, result) {
							fnKendoMessage({
								...
								ok : function() {
									...
									if(pageMode == "create") {
										var successUrl = "/layout.html#/goodsMgm?ilGoodsId="+data['ilGoodsId'];
										location.href = successUrl;
									}
									else{
						...
						, fail : function(data, code) {
							switch (code.code) {
								...
								case 'DELETE_GOODS_RESERVATION_OPTION_FAIL':	// 삭제 불가한 예약상품이 있습니다. 예약 회차를 다시 확인해주세요.
									fnKendoMessage({
