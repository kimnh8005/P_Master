...
$(document).ready(function() {
	...
	function fnAddExcelSubmit(){
        let url  = "/admin/goods/discount/addEmployeeExcelUpload";
	...
	function fnSamepleFormDownload(){
		document.location.href = "/contents/excelsample/goodsdiscount/sampleGoodsPriceDiscountEmployee.xlsx"
	};
    ...
    function fnGetDataList(){
        ...
        if( aGridCnt > 0 ){
            for(let i = 0; i < aGridCnt; i++){
                ...
                orgData.discountTypeCode      	= aGridData[i].DISCOUNT_TP;
                orgData.discountMethodTypeCode  = aGridData[i].DISCOUNT_METHOD_TP;
    ...
    function fnClose() {
      ...
      option.url = "#/goodsDisEmpUploadList";// ERP 재고 엑셀 업로드 내역 화면 URL
      option.menuId = 1025;// ERP 재고 엑셀 업로드 내역 화면 메뉴 ID
