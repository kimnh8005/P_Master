...
$(document).ready(function() {
	...
	function fnUI() {
		...
		fnInitCompont();		// 매장전용 상품 컴포넌트 Initialize
		fnViewModelInit();
...
//            },
            ...
            fnGoodsLink  : function(rowData, ctrlKey){ // 매장 상품 상세이동
...
//        viewModel.fnCompanyStandardTypeChange();
  ...
  function fnItemGrid() {
		...
		itemGridDs = fnGetEditPagingDataSource({
			url : '/admin/goods/list/getGoodsShopList',
			pageSize : PAGE_SIZE,
		...
		itemGridOpt = {
			...
			navigatable: true,
			columns : [
				{ field : 'itemCd',  title : '품목코드<BR>품목바코드', width : '140px', attributes : { style : 'text-align:center' },
            ...
            ,   { field : 'goodsTpNm', title : '상품유형', width : '60px', attributes : { style : 'text-align:center' }}
			...
			,	{ field : 'categoryStandardDepthName', title : '표준 카테고리', width : '170px', attributes : { style : 'text-align:center' } }
...
//		}
	...
	function fnExcelExport() {
		...
		var data = $('#searchForm').formSerialize(true);
		fnExcelDownload('/admin/goods/list/goodShopExportExcel', data);
	}
