...
$(document).ready(function() {
	...
	function fnItemGrid() {
		...
		itemGridDs = fnGetEditPagingDataSource({
			url : '/admin/goods/discount/getGoodsDiscountUploadList',
			pageSize : PAGE_SIZE,
    ...
    $('#itemGrid').on("click", "button[kind=btnDtl]", function(e) {
      ...
      option.url = "#/goodsDiscountUploadDtlList";// ERP 재고 엑셀 업로드 내역 화면 URL
      option.menuId = 1019;// ERP 재고 엑셀 업로드 내역 화면 메뉴 ID
    ...
    $('#itemGrid').on("click", "button[kind=btnExcelDown]", function(e) {
		...
		fnExcelDownload('/admin/goods/discount/createGoodsDiscountUploadFailList', data);
