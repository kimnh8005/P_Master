...
var excepTemplateDefaultParams = { "excelTemplateTp" : "EXCEL_TEMPLATE_TP.ORDER"};
...
$(document).ready(function() {
	...
	function fnClear(){
		...
		aGridDs.read({"stCommonCodeMasterId" : "EXCEL_TEMPLATE_ORDER", "useYn" : "Y"});
	...
	function fnSave(){
		...
		*/
		...
		data["urCompanyId"] =  "1";
		data["excelTemplateTp"] =  "EXCEL_TEMPLATE_TP.ORDER";
		data["excelTemplateUseTp"] =  "EXCEL_TEMPLATE_USE_TP.DOWNLOAD";
		...
		*/
	    ...
	    // 개인정보 포함여부 체크
		var privacyIncludeYn = "N";
		for (var i = 0; i < jsonData.length; i++) {
			if(jsonData[i].ATTRIBUTE2 == 'Y') { // 공통코드 추가 속성값 2
				privacyIncludeYn = "Y";
				break;
			}
		}
		data["privacyIncludeYn"] = privacyIncludeYn;
	...
	function fnInitOptionBox(){
		...
		fnTagMkChkBox({
	        id    : 'accountFormYn',
	        data  : [{ "CODE" : 'Y' , "NAME" : "계정별 양식" }],
	        tagId : 'accountFormYn',
	        chkVal: 'N',
	        style : {}
	    });

		//$('input:checkbox[name=accountFormYn]').prop("checked",false);
		$('input:checkbox[name=accountFormYn]').on("change",  function(index){

			if($('input:checkbox[name=accountFormYn]:checked').val() == 'Y'){
				var params = { "excelTemplateTp" : "EXCEL_TEMPLATE_TP.ORDER", "accountFormYn" : "Y"}
				fnExcelDropDownList(params, "Y");
			}else{
				fnExcelDropDownList(excepTemplateDefaultParams, "N");
			}
...
//		});
	...
	function fnInitGrid(){
		...
		aGridOpt = {
			...
			,columns : [
				 { field : 'dictionaryMasterName'	, title : '항목명'		, width : '50%', attributes:{ style:'text-align:left' }}
				...
				,{ field : 'attribute2'				, title : '개인정보 보유 여부'	, width : '25%', attributes:{ style:'text-align:center' }
					, template : function(dataItem){ return dataItem.attribute2 == "Y" ? "●": "";}}
			]
		};
		...
		bGridOpt = {
			...
			,columns   : [
				...
				,{ field : 'NAME'			, title : '선택항목(Title)'	, width : '50%', attributes : {style:'text-align:left'}}
				,{ field : 'ATTRIBUTE1'		, title : '입력필수항목'		, width : '15%', attributes : { style:'text-align:center' }
						, template : function(dataItem){ return dataItem.ATTRIBUTE1 == "Y" ? "●": "";}}
				,{ field : 'ATTRIBUTE2'				, title : '개인정보 보유 여부'	, width : '15%', attributes:{ style:'text-align:center' }
					, template : function(dataItem){ return dataItem.ATTRIBUTE2 == "Y" ? "●": "";}}
			]
	...
	function fnGridAddItem(idx){
		...
		if( data == null || data == undefined){
			...
			jObj['ATTRIBUTE2'] = data.attribute2;
...
//		}
	...
	function fnExcelDropDownList(params, accountFormYn){
        fnKendoDropDownList({
