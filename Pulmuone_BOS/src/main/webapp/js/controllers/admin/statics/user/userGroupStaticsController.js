...
$(document).ready(function() {
  ...
  function fnDefaultSet(){
    ...
    function fnSetUserGroupInfo() {
        let urGroupMasterId = $("#userMaster").val();
        if(urGroupMasterId == "") {
            $('#userGroupInfo').text("");
            return;
        }
    	fnAjax({
            ...
            success : function( data ){
                ...
                let userGroupInfo = data.rows[0].startDate + " ~ ";
                if(nowDate < endDate){
                    userGroupInfo += "진행중";
                } else {
                    userGroupInfo += data.rows[0].endDate;
                }

                $('#userGroupInfo').text(userGroupInfo);
            },
            isAction : "select"
        });
    }
  ...
  function fnInitOptionBox() {
    ...
    $('#userMaster').bind("change", fnSetUserGroupInfo);
  ...
  function fnSearch() {
    ...
    // Validation
    // ------------------------------------------------------------------------
    let userMaster         = $('#userMaster').val();
    if (fnIsEmpty(userMaster)) {
      fnMessage('', '<font color="#FF1A1A">[회원등급]</font> 필수 입력입니다.', 'startDe');
      return;
    }
  ...
  function fnInitGrid(){
    ...
    gridDs = fnGetPagingDataSource({
        url : '/admin/statics/user/getUserGroupStaticsList'
      //, pageSize : PAGE_SIZE
    });
    ...
    gridOpt  = {
      ...
      , autobind    : false
      , columns     : [
            { field:'groupName', title: '구분', width:120 , attributes:{ style:'text-align:center' }}
          , { field:'userCount', title: '구매고객 수', width:120 , attributes:{ style:'text-align:center' }, format: '{0:\#\#,\#}'}
          , { field:'paidPrice', title: '판매금액', width:120 , attributes:{ style:'text-align:center' }, format: '{0:\#\#,\#}'}
          , { field:'orderCount', title: '주문건수', width:120 , attributes:{ style:'text-align:center' }, format: '{0:\#\#,\#}'}
          , { field:'goodsCount', title: '주문 상품수량', width:120 , attributes:{ style:'text-align:center' }, format: '{0:\#\#,\#}'}
          ]
    };
  ...
  function fnExcelDownSaleStaticsList(gubn, keyVal) {
    ...
    inputData.userMasterName = $('#userMaster').data('kendoDropDownList').text();
    inputData.agentTypeCdName = fnGetCheckBoxText('agentTypeCd');
    let url         = '/admin/statics/user/getUserGroupStaticsExcelDownload';
