package kr.co.pulmuone.mall.base.controller;
...
@RestController
public class StorageMallController {
    ...
    public ApiResult<?> uploadMultiplePublicFiles(MultiFileUploadRequestDto multiFileUploadRequestDto) {
        ...
        String rootStoragePath = MallStorageInfoEnum.getRootStoragePath( //
                multiFileUploadRequestDto.getStorageType() //
        );
        /* BosStorageInfoEnum 참조하여 하위 디렉토리 ( 1차 (모듈별) / 2차 (도메인별) / 3차 (시간별) ) 경로 산정 */
        String fullSubStoragePath = MallStorageInfoEnum.getFullSubStoragePath( //
     ...
     *
     * (dev) http://slb-4328295.ncloudslb.com/ 반환
     *
